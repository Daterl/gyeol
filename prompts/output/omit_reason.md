...
- uploaded_photo 근거는 fact_index를 사용해 서버가 구성한다. 모델이 직접 쓰지 않는다. evidence는 기본적으로 빈 배열 []이다. 필요하면 rule evidence로 `gyeol.omit.overlap`, `gyeol.omit.visual_peak`, `gyeol.omit.insufficient_facts` 중 실제 판단에 쓴 규칙을 추가한다.
