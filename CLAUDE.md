...
```
npm run verify:deployed                     # 기본 공개 URL
...
| 시점 | 무엇을 하나 |
|---|---|
| **`develop` 에 머지한 직후** | 돌린다. 실패하면 **원인이 내 변경인지 배포 지연인지부터 가른다** |
| **배포가 갱신됐다고 들었을 때** | 돌린다. 말이 아니라 응답으로 확인한다 |
| **하루 최소 1회** | 돌린다. 아무도 안 건드려도 배포는 썩는다 |
| **제출 전** | 돌린다. 여기서 실패하면 제출물에 그 기능을 쓰지 않는다 |

### 실패했을 때 — 원인을 먼저 가른다

```
1. Production ref 와 origin/develop 을 대조한다
   gh api repos/Daterl/gyeol/deployments --jq '[.[]|select(.environment=="Production")][0].ref'
   git rev-list --count <ref>..origin/develop

2. 몇 커밋이라도 뒤처졌으면 → 배포 지연이다. 디에고 이슈.
   0 이면 → 코드 문제다. 내가 고친다.
```

**이 순서를 지켜라.** 배포 지연을 코드 버그로 오해하면 없는 버그를 몇 시간 쫓는다.

### 라우트 404 는 특별히 본다

`/api/analyze`, `/api/generate` 가 **404** 면 라우트 파일이 배포에 없다는 뜻이다(405 는 정상 — 라우트는 있고 메서드만 다름).
소스에 `src/app/api/<name>/route.ts` 가 있는데 404 면 **거의 항상 배포 지연**이다.
