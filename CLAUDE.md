...
## 브랜치 전략 (2026-09-17 재조정) — **main 은 배포 브랜치다**

`main` 에 Vercel 배포가 걸려 있다. **main 에 머지될 때마다 배포가 돈다.**
그래서 작업을 main 에 직접 쌓지 않는다.

```
feature/*  →  dev   (평소 작업은 전부 여기로 모인다)
               ↓
              main  (배포할 때만. 의도적으로 연다)
```

| 브랜치 | 무엇인가 | 누가 언제 |
|---|---|---|
| `main` | **배포 브랜치.** 머지되면 Vercel 배포가 돈다 | 배포할 시점에만. `dev → main` PR 을 따로 연다 |
| `dev` | 작업 통합 브랜치. 모든 작업의 기본 목적지 | 모든 feature PR 의 base |
| `feat/<이슈번호>-<슬러그>` | 이슈 하나당 하나 | 작업 시작할 때 **dev 에서** 갈라낸다 |

### 지켜야 할 것
- feature 브랜치는 **`origin/dev` 에서** 갈라내고 **`dev` 로** PR 을 연다.
  ```
  git fetch origin
  git worktree add ../gyeol-<N> -b feat/<N>-<슬러그> origin/dev
  gh pr create --base dev --draft ...
  ```
  `gh pr create` 는 base 를 생략하면 **기본 브랜치(main)로 간다.** `--base dev` 를 반드시 붙인다.
- **rebase 대상도 `origin/dev`** 다. `origin/main` 이 아니다.
- **main 으로 직접 PR 을 열지 않는다.** 배포를 의도한 것이 아니라면.
- `dev → main` 은 **배포 판정**이라 자동 머지 대상이 아니다. 코디네이터가 배포 시점을 따로 정한다.

⚠️ GitHub 기본 브랜치는 `main` 으로 둔다. Vercel 의 프로덕션 브랜치 설정이 기본 브랜치를 따라갈 수 있어,
바꾸면 배포가 dev 로 옮겨갈 위험이 있다. **대신 PR 마다 `--base dev` 를 명시한다.**
...
⑤ 지적이 있으면 고친다 (한 PR 당 최대 3회)
     ↓
⑥ 판정
     ├ BLOCKER·HIGH 0건 + 테스트 통과 → **dev 로** merge 하고 이슈 닫기
...
- 수정한 뒤에는 **고치기 전엔 통과하고 고친 뒤엔 실패하는지**를 보인다. 통과만 확인하는 것은 증거가 아니다.

### merge 판단 기준 (자동)
아래를 **전부** 만족하면 사람을 기다리지 않고 dev 로 merge 한다.
...
| 루프 지점 | 보드 상태 | 같이 남기는 것 |
|---|---|---|
| ① 이슈를 고른 직후 | 진행 중 | 이슈 댓글에 "작업 시작" 과 워크트리·브랜치 이름 |
| ③ Draft PR 을 연 직후 | 리뷰 중 | 이슈 댓글에 PR 링크 |
| ⑤ 지적을 고치는 동안 | 리뷰 중 유지 | 몇 번째 수정인지 |
| ⑥ merge 함 | 완료 | 이슈를 닫는다 |
