...
/** A title must not promote one photo's on-image text — no digits, no lifted phrase. */
export function scoreTitle(title, photos) {
  const value = String(title ?? '');
  const lifted = [];
  for (const photo of photos) {
    const overlay = String(photo.text_in_image ?? '');
    for (let i = 0; i + 5 <= overlay.length; i++) {
      const piece = overlay.slice(i, i + 5);
      if (value.includes(piece)) lifted.push({photo_id: photo.photo_id, piece});
    }
  }
  return {value, digits: value.match(/\d+/g) ?? [], lifted_overlay: lifted};
}
...
export function validateArtifact(a) {
  ...
  assert.equal(a.acceptance.live_model, 'PASS');
  for (const row of a.review) {
    assert.ok(a.photos.some(p => p.photo_id === row.photo_id), 'Review row must name a recorded photo');
...
생성: LIVE (${a.provenance.model.output}) · 사진 분석: LIVE (${a.provenance.model.vision}) · 사람 게시 가능 판정: **PENDING**

에이전트가 원본 이미지 15장을 직접 열어 판정한 결과다. **사람 판정을 대체하지 않는다.** 사람 칸은 비워 둔다.

| 합계 | 값 |
|---|---|
| 성공 회차 / 시도 | ${s.totals.rounds} / ${s.totals.rounds + a.provenance.generate_failures.length} |
| 에이전트 판정 — 쓸 수 있음 / 근거 있는 비움 / 못 씀 | ${t.usable} / ${t.omitted_ok} / ${t.not_usable} (단서 슬롯 ${t.seed_slots}개 중 과반: ${t.majority ? '충족' : '미충족'}) |
| 단서 / 비움 슬롯 | ${s.totals.seeds} / ${s.totals.omitted} |
| 금지어·단정 | ${s.totals.banned} |
| 소재가 그 슬롯 사실에 없음 (S4) | ${s.totals.unsupported_materials} |
...
export async function runLive({imageDir, attempts = 4, analyzed}) {
  ...
  return {
    ...
    acceptance: {live_model: 'PASS', human: 'PENDING'},
