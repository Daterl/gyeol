...
export function createProfileRequestLimit({storage,now=Date.now,limits=PROFILE_LIMITS,sessionLimits=SESSION_LIMITS}={}) {
  async function consume(key,maximum) {
    ...
    for(let attempt=0;attempt<8;attempt++) {
      ...
      if(row && (!row.etag || row.value?.version!==1 || !Number.isSafeInteger(row.value.window)
        || !Number.isSafeInteger(row.value.count) || row.value.count<0 || row.value.window>window)) throw limitUnavailable();
    ...
    throw limitUnavailable();
  }
  return {async take(action,sessionId) {
    if(!Object.hasOwn(limits,action) || !Number.isSafeInteger(limits[action]) || limits[action]<1) throw limitUnavailable();
    try {
      // Global consumed first: rotating sessions, spoofed IPs and CAS losers cannot
      // increase the provider budget. Partial reservations are never refunded.
      ...
      if(sessionId && Object.hasOwn(sessionLimits,action)) {
        // 256 fixed buckets per action; a collision only tightens the limit.
        const bucket=digest(sessionId).slice(0,2);
        await consume(`profile-request-limit/v1/${digest(action+':'+bucket)}.json`,sessionLimits[action]);
      }
    } catch(error) {if(error instanceof RequestError) throw error;throw limitUnavailable();}
  }};
}
