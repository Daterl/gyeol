...
const readReceipt = (receipt, secret, now) => {
  ...
  const expected = createHmac('sha256', secret)
    .update(body ?? '')
    .digest('base64url');
  fail(!extra && safeEqual(signature, expected), 'INVALID_RECEIPT');
  let value;
  try {
...
const validateCuration = (curation, receipt) => {
  ...
  if (curation.includeProfile !== true) {
    fail(curation.profile === undefined);
    return { photos: curation.photos, includeProfile: false };
  }
  const profile = curation.profile;
  fail(
    exactKeys(profile, [
      'username',
      'displayName',
      ...
      /^[A-Za-z0-9._]{1,30}$/.test(profile.username) &&
      (profile.displayName === null ||
        (typeof profile.displayName === 'string' && profile.displayName.length <= 100)) &&
      (profile.avatarUrl === null ||
        (typeof profile.avatarUrl === 'string' &&
          profile.avatarUrl.length <= 2048 &&
          /^https:\/\//.test(profile.avatarUrl))) &&
      typeof profile.source === 'string' &&
      profile.source.length <= 2048 &&
      typeof profile.collectedAt === 'string' &&
      !Number.isNaN(Date.parse(profile.collectedAt)),
  );
  return { photos: curation.photos, includeProfile: true, profile };
};
...
export function createShareService({
  ...
  return {
    ...
    async uploadPhoto({ receipt, uploadToken, photoId, contentType, body } = {}) {
      ...
      const session = parse(marker);
      fail(session.expiresAt >= now() && session.photoIds.includes(photoId), 'INVALID_SESSION');
    ...
    async publish({ receipt, uploadToken, managementKey, curation, ifMatch } = {}) {
      const value = readReceipt(receipt, secret, now());
      const receiptHash = sha256(receipt);
      fail(safeEqual(uploadToken, sessionToken(receiptHash)), 'UNAUTHORIZED');
      fail(
      ...
      const published = await store.put(manifestPath(value.shareId), json(manifest), {
        contentType: 'application/json',
        ...(current ? { ifMatch } : { ifNoneMatch: true }),
      });
      await deletePrefix(session.prefix);
      return { shareId: value.shareId, version: value.version, etag: published.etag };
    },
    ...
    async readImage(shareId, photoId) {
      ...
      fail(value.status === 'active', 'GONE');
      const object = value.objects[photoId];
      fail(
        object &&
    ...
    async rotateKey({ shareId, managementKey, ifMatch, caller } = {}) {
      limit('share-manage', caller);
      const { blob, value } = await getManifest(shareId);
      fail(value.status === 'active', 'GONE');
      authorize(value, managementKey);
      fail(ifMatch === blob.etag, 'CONFLICT');
      const nextKey = randomToken(32);
      const updated = { ...value, managementKeyHash: sha256(nextKey) };
      const result = await store.put(manifestPath(shareId), json(updated), {
        contentType: 'application/json',
        ifMatch,
      });
      return { managementKey: nextKey, etag: result.etag };
    },
    ...
    async revoke({ shareId, managementKey, ifMatch, caller } = {}) {
      ...
      const result = await store.put(manifestPath(shareId), json(tombstone), {
        contentType: 'application/json',
        ifMatch,
      });
