...
export function createShareService({
  ...
  //
  // This is weaker than the conditional write it replaces, and the gap is real: two writers that
  // start from the same revision can BOTH re-read their own value and BOTH return success, with
  // the one that landed first silently overwritten. A caller can be told its key rotation
  // succeeded while a concurrent rotation is the one that survives. Sequential callers and a
  // single writer are unaffected; only genuinely concurrent management writes on one share are.
  // Accepted deliberately because the alternative shipped today is that rotate and revoke never
  // work at all. Restoring a real compare-and-set needs a store primitive this one does not
  // offer — tracked in the follow-up issue on #210, not fixed by tightening this function.
