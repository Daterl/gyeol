...
const LEXICON = [
  ...
  { field: 'caption_coverage', words: ['말수가 적', '말수 적게', '말수는 적게', '일부는 비워', '사진만 두', '몇 장만', '몇 장에만'], value: 'sparse', confidence: 0.8, row: '말수가 적·말수 적게·일부는 비워·사진만 두·몇 장만 → sparse' },
  { field: 'caption_coverage', words: ['모든 사진에 문장', '모든 사진에 짧게', '모든 사진에 써', '사진마다', '한 장도 비우지', '전부 써', '전부 채워'], value: 'all', confidence: 0.8, row: '모든 사진에 문장·모든 사진에 써·사진마다·한 장도 비우지·전부 써·전부 채워 → all' }
];
const UNSUPPORTED_COVERAGE = ['캡션 없이', '전부 사진만'];
...
const coverageNegates = (clause, phrase) => {
  if (phrase === '한 장도 비우지') return false;
  const after = clause.slice(clause.indexOf(phrase) + phrase.length, clause.indexOf(phrase) + phrase.length + 20);
  return /(말고|말구|말아|지\s*(?:마|않)|고\s*싶지\s*않|싫|아니|별로|반대|안\s)/u.test(after);
};
...
export function extractFromFreetext(text, { profileId, createdAt } = {}) {
  ...
  const language = {};
  const coverageEntries = LEXICON.filter(entry => entry.field === 'caption_coverage');
  const coverageMatches = coverageEntries.flatMap(entry => entry.words.flatMap(word =>
    parts.filter(part => part.includes(word)).map(part => ({entry,word,negated:coverageNegates(part,word),index:raw.lastIndexOf(word)}))));
  const unsupportedCoverage = UNSUPPORTED_COVERAGE.filter(word => parts.some(part => part.includes(word)));
  // 1차: 부정된 어구가 가리키는 항목을 먼저 봉쇄한다. 순서에 좌우되지 않게 하려고 두 번 돈다.
  const blocked = new Set();
  for (const entry of LEXICON.filter(entry => entry.field !== 'caption_coverage')) for (const w of entry.words) {
  ...
  // 2차: 봉쇄되지 않은 항목만 채운다.
  for (const entry of LEXICON.filter(entry => entry.field !== 'caption_coverage')) {
  ...
  if (unsupportedCoverage.length === 0 && coverageValues.size > 0) {
    let candidates = coverageMatches.filter(match => !match.negated);
    if (coverageValues.size > 1 && raw.includes('하지만')) {
      const contrastAt = raw.lastIndexOf('하지만');
      const afterContrast = candidates.filter(match => match.index > contrastAt);
      if (new Set(afterContrast.map(match => match.entry.value)).size === 1) candidates = afterContrast;
    }
    if (new Set(candidates.map(match => match.entry.value)).size === 1) {
      const selected = candidates[0];
      language.caption_coverage = claim(selected.entry.value, selected.entry.confidence,
        [textEvidence(pid, selected.word), ruleEvidence(selected.entry.row)]);
    }
  }
