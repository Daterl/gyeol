...
const COVERAGE_PATTERNS = [
  { pattern: /말수가\s*적고(?=\s|[,·.!?]|$)/gu, value: 'sparse', row: '말수가 적고 → sparse' },
  { pattern: /말수(?:는)?\s*적게\s*(?:(?:기록|문장|캡션)(?:을|를)?\s*)?(?:해|써)\s*(?:줘|주세요)(?=$|[\s,·.!?])/gu, value: 'sparse', row: '말수 적게 기록해 달라는 요청 → sparse' },
  { pattern: /일부(?:는)?\s*(?:사진|장면|캡션)?\s*비워\s*(?:줘|주세요)(?=$|[\s,·.!?])/gu, value: 'sparse', row: '일부는 비워 달라는 요청 → sparse' },
  { pattern: /사진만\s*(?:두고\s*싶(?:어|어요)|둬\s*(?:줘|주세요)|두어\s*(?:줘|주세요))(?=$|[\s,·.!?])/gu, value: 'sparse', row: '사진만 두고 싶다는 요청 → sparse' },
  { pattern: /몇\s*장(?:만|에만)\s*(?:(?:문장|캡션|글|한\s*줄)(?:을|를)?\s*|자세히\s*)?써\s*(?:줘|주세요)(?=$|[\s,·.!?])/gu, value: 'sparse', row: '몇 장에만 문장·글을 써 달라는 요청 → sparse' },
  { pattern: /모든\s*사진에\s*(?:(?:문장|캡션|글|한\s*줄)(?:을|를)?\s*|짧게\s*)?써\s*(?:줘|주세요)(?=$|[\s,·.!?])/gu, value: 'all', row: '모든 사진에 문장·글을 써 달라는 요청 → all' },
  { pattern: /^사진마다\s*한\s*줄씩$/gu, value: 'all', row: '사진마다 한 줄씩 → all' },
  { pattern: /사진마다\s*(?:(?:한\s*줄|문장|캡션|글)(?:을|를|씩)?\s*)?써\s*(?:줘|주세요)(?=$|[\s,·.!?])/gu, value: 'all', row: '사진마다 문장·글을 써 달라는 요청 → all' },
  { pattern: /한\s*장도\s*비우지\s*않았으면\s*해(?:요)?(?=$|[\s,·.!?])/gu, value: 'all', row: '한 장도 비우지 않았으면 한다는 요청 → all', intrinsicNegative: true },
  { pattern: /(?:^(?:(?:캡션|문장)(?:을|를)?\s+)?|한\s*장도\s*비우지\s*말고\s+)전부\s*(?:써|채워)\s*(?:줘|주세요)(?=$|[\s,·.!?])/gu, value: 'all', row: '전부 써·채워 달라는 요청 → all' }
];
const NEGATED_COVERAGE_PATTERNS = [
  { pattern: /사진만\s*두지\s*마/gu, value: 'sparse' },
  { pattern: /몇\s*장(?:만|에만)\s*(?:(?:문장|캡션|글|한\s*줄)(?:을|를)?\s*)?(?:쓰지(?:는)?|써\s*주지는?)\s*마/gu, value: 'sparse' },
  { pattern: /(?:모든\s*사진에|사진마다)\s*(?:(?:문장|캡션|글|한\s*줄)(?:을|를)?\s*)?(?:쓰지(?:는)?|써\s*주지는?)\s*마/gu, value: 'all' },
  { pattern: /전부\s*(?:쓰지(?:는)?|써\s*주지는?|채우지(?:는)?|채워\s*주지는?)\s*마/gu, value: 'all' }
];
const UNSUPPORTED_COVERAGE = /(?:캡션\s*없이|전부\s*사진만)/u;
const COVERAGE_REJECTION = /(싫|원하지\s*않|말아야|아니|반대|별로|싶지\s*않)/u;
const COVERAGE_CORRECTION = /^(?:아니요|아뇨|아니)(?:\s+|$)/u;
...
const coverageSegments = text => {
  ...
  const add = end => {
    ...
    segments.push({text:value,index:cursor+leading,contrast});
    return true;
  };
  for (const match of text.matchAll(separator)) {
    const added = add(match.index);
    cursor = match.index + match[0].length;
    if(match[0] === '하지만') contrast = true;
    else if(added) contrast = false;
  }
  add(text.length);
  return segments;
};
const coverageNegates = (clause, match, intrinsicNegative) => {
  const checked = clause.replace(COVERAGE_CORRECTION,'');
  if (COVERAGE_REJECTION.test(checked)) return true;
  if (intrinsicNegative) return false;
  return /(지(?:는)?\s*(?:마|않)|(?:고\s*)?싶지(?:는)?\s*않|말구|말아|아니)/u.test(checked.replace(match[0],' '));
};
...
export function extractFromFreetext(text, { profileId, createdAt } = {}) {
  ...
  let unsupportedCoverage = false;
  for (const part of coverageParts) {
    const events = [
      ...COVERAGE_PATTERNS.flatMap(entry => [...part.text.matchAll(entry.pattern)].map(match => ({
        entry,phrase:match[0],index:part.index+match.index,
        negated:coverageNegates(part.text,match,entry.intrinsicNegative)
      }))),
      ...NEGATED_COVERAGE_PATTERNS.flatMap(entry => [...part.text.matchAll(entry.pattern)]
        .map(match => ({entry,phrase:match[0],index:part.index+match.index,negated:true})))
    ].sort((a,b)=>a.index-b.index);
    const unsupported = UNSUPPORTED_COVERAGE.test(part.text);
    if (COVERAGE_CORRECTION.test(part.text) || (part.contrast && (events.length>0 || unsupported))) activeCoverage.clear();
    for (const event of events) {
      if(event.negated) activeCoverage.delete(event.entry.value);
      else activeCoverage.set(event.entry.value,event);
    }
    unsupportedCoverage ||= unsupported;
  }
  const coverageValues = new Set(activeCoverage.keys());
  ...
  if (!unsupportedCoverage && coverageValues.size === 1) {
    const candidates = [...activeCoverage.values()];
