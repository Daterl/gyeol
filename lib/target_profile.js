...
const COVERAGE_PATTERNS = [
  ...
  { pattern: /사진마다\s*한\s*줄씩(?=$|[\s,·.!?])/gu, value: 'all', row: '사진마다 한 줄씩 → all' },
  ...
  { pattern: /전부\s*(?:써|채워)\s*(?:줘|주세요)(?=$|[\s,·.!?])/gu, value: 'all', row: '전부 써·채워 달라는 요청 → all' }
];
const NEGATED_COVERAGE_PATTERNS = [
  /사진만\s*두지\s*마/gu,
  /(?:모든\s*사진에|사진마다|몇\s*장(?:만|에만))\s*(?:(?:문장|캡션|글|한\s*줄)(?:을|를)?\s*)?(?:쓰지|써\s*주지는?)\s*마/gu,
  /전부\s*(?:쓰지|써\s*주지는?|채우지|채워\s*주지는?)\s*마/gu
];
...
const hasCoverageCue = text => [...COVERAGE_PATTERNS.map(entry=>entry.pattern),...NEGATED_COVERAGE_PATTERNS]
  .some(pattern=>{
    pattern.lastIndex=0;
