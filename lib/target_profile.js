...
import { ContractError, validateClaim, validateProfile, validatePhoto } from './contracts.js';
...
const toneWords = text => text.split(/[,·\n]|그리고|하고\s/u).map(s => s.trim().replace(/[.!?]+$/u, ''))
  .filter(s => s.length > 0).map(s => s.split(/\s+/u).slice(0, 2).join(' ')).slice(0, 5);
...
export function extractFromFreetext(text, { profileId, createdAt } = {}) {
  ...
  for (const entry of LEXICON) {
    if (Object.hasOwn(language, entry.field)) continue;
    const phrase = entry.words.find(w => raw.includes(w));
    if (!phrase) continue;
...
const photoEvidence = (analyses, note, picks) => [
  ...
  ...picks.map(a => ({ kind: 'uploaded_photo', ref: a.photo_id, note: `${a.scale}·${a.composition}` }))
];
...
export function validatePhotoPlan(v) {
  const p = 'PhotoPlan';
  if (v === null || typeof v !== 'object' || Array.isArray(v)) throw new ContractError(p, 'expected object');
  if (v.kind !== 'photo_plan') throw new ContractError(p + '.kind', 'expected photo_plan');
  if (v.source !== 'photo_only') throw new ContractError(p + '.source', 'expected photo_only');
  if (Object.hasOwn(v, 'axis') || Object.hasOwn(v, 'present')) throw new ContractError(p, 'photo plan is not a TargetProfile');
  if (v.target_profile !== null) throw new ContractError(p + '.target_profile', 'must be null');
  if (v.language !== null) throw new ContractError(p + '.language', 'photographs carry no language evidence');
  if (!Number.isInteger(v.sample_size) || v.sample_size < 1) throw new ContractError(p + '.sample_size', 'expected integer >= 1');
  if (typeof v.disclaimer !== 'string' || v.disclaimer.trim().length === 0) throw new ContractError(p + '.disclaimer', 'expected nonempty string');
  if (v.completeness?.language !== 0 || v.completeness?.sequence !== 0) throw new ContractError(p + '.completeness', 'language and sequence must be zero');
  for (const [k, c] of Object.entries(v.visual)) validateClaim(c, `${p}.visual.${k}`, true);
  return v;
}
