...
const COVERAGE_CORRECTION = /(?:아니요|아뇨).*?(?:원하지\s*않|싫|반대|별로)/u;
...
const hasCoverageCue = text => [...COVERAGE_PATTERNS,...NEGATED_COVERAGE_PATTERNS].map(entry=>entry.pattern)
  .some(pattern=>{
    pattern.lastIndex=0;
    const matched=pattern.test(text.trim());
    pattern.lastIndex=0;
    return matched;
  }) || UNSUPPORTED_COVERAGE.test(text);
const coverageSegments = text => {
  const marker = '하지만';
  const contrast = text.lastIndexOf(marker);
  const afterContrast = contrast < 0 ? '' : text.slice(contrast + marker.length);
  const offset = contrast >= 0 && hasCoverageCue(afterContrast) ? contrast + marker.length : 0;
...
const coverageNegates = (clause, match, intrinsicNegative) => {
  ...
  if (COVERAGE_REJECTION.test(clause)) return true;
  if (intrinsicNegative) return false;
  return /(지(?:는)?\s*(?:마|않)|(?:고\s*)?싶지(?:는)?\s*않|말구|말아|아니)/u.test(before+after);
};
...
export function extractFromFreetext(text, { profileId, createdAt } = {}) {
  ...
  const coverageEvents = [
    ...coverageMatches,
    ...coverageParts.flatMap(part => NEGATED_COVERAGE_PATTERNS.flatMap(entry =>
      [...part.text.matchAll(entry.pattern)].map(match => ({entry,phrase:match[0],index:part.index+match.index,negated:true})))),
    ...coverageParts.filter(part=>COVERAGE_CORRECTION.test(part.text)).map(part=>({index:part.index,clear:true}))
  ].sort((a,b)=>a.index-b.index);
  const activeCoverage = new Map();
  for (const event of coverageEvents) {
    if(event.clear) activeCoverage.clear();
    else if(event.negated) activeCoverage.delete(event.entry.value);
