...
import { OMIT_RULES, withOmitSuggestions } from './omit-suggestion.js';
import { SIGNATURE_SIDE } from './photo_signature.js';
...
export function validatePhoto(v) {
  ...
  // 선택 관측값. 있으면 픽셀 측정에서 나온 고정 길이 서명이어야 하고, 없으면 그 사진에 구조가 관측되지
  // 않은 것이다 — 길이·값이 어긋난 서명은 관측이라고 부를 수 없으므로 거부한다(#97).
  if(v.structure_signature!==undefined) {
    array(v.structure_signature,p+'.structure_signature');
    ok(v.structure_signature.length===SIGNATURE_SIDE*SIGNATURE_SIDE,p+'.structure_signature','must have '+SIGNATURE_SIDE*SIGNATURE_SIDE+' cells');
    v.structure_signature.forEach(cell=>ok(typeof cell==='number'&&Number.isFinite(cell),p+'.structure_signature','cells must be finite numbers'));
  }
  return v;
}
...
export function validateFeed(v, inputPhotoIds, currentProfile, targetProfile, photoAnalyses, currentPhotoIds = []) {
  ...
  if (Object.hasOwn(v,'omit_summary') || v.slots.some(s=>Object.hasOwn(s,'omit_suggestion'))) {
    // 어느 규칙 집합으로 만든 응답인지는 응답에 적혀 있지 않다. 그래서 지원하는 규칙을 낮은 것부터
    // 전부 재계산해 보고 하나라도 정확히 맞으면 통과시킨다. 위조는 여전히 막힌다 — 모든 후보가 실제
    // PhotoAnalysis 에서 재계산된 것이므로, 없는 권고를 지어내면 어느 쪽과도 맞지 않는다. 반대로 권고를
    // 빼는 방향(규칙 1)은 "권고가 없는 상태" 이고 그건 원래 정상이다(P3).
    // 이 관용이 배포·롤백 양방향의 혼합 버전 창을 연다 — 옛 번들은 규칙 1 응답을, 새 번들은 옛 서버의
    // 규칙 1 응답을 각각 받아들인다.
    let failure=null;
    const accepted=OMIT_RULES.some(rules=>{ try { checkOmit(withOmitSuggestions(v,photoAnalyses,{rules})); return true; }
      catch(error) { failure=error; return false; } });
    if (!accepted) throw failure;
  }
  function checkOmit(expected) {
...
export function validateCaptionSlot(s, inputPhotoIds) {
  ...
  oneOf(s.caption_state,['seed','omitted','user'],'caption_state');
  if(s.caption_state==='omitted') { ok(s.text===null,'text','omitted text must be null'); text(s.omit_reason,'omit_reason'); }
  else { text(s.text,'text'); ok(s.omit_reason===null,'omit_reason','seed/user reason must be null'); }
