...
export function validateTargetAxis(applied, targetProfile) {
  object(applied,'applied_profile'); text(applied.target_profile_id,'target_profile_id');
...
export function validatePhotoRefs(v, inputPhotoIds, p = '$') {
  const allowed=new Set(validateInputIds(inputPhotoIds));
...
export function validateFeed(v, inputPhotoIds, currentProfile, targetProfile, photoAnalyses) {
  ...
  validatePhotoRefs(v,inputPhotoIds,'OrderedFeed'); validateFactProvenance(v,photoAnalyses);
  validateClaims(v); return v;
}
...
export function validateExport(v, feed, inputPhotoIds) {
  ...
  for(const s of v.slots) {
    oneOf(s.caption_state,['filled','omitted','user'],'caption_state'); if(s.caption_state==='omitted') ok(s.text===null,'text','omitted text must be null'); else text(s.text,'text');
    array(s.evidence,'export.evidence'); ok(s.evidence.length>0,'export.evidence','needs evidence'); s.evidence.forEach(e=>validateEvidence(e));
    if(s.caption_state==='user') ok(s.evidence.some(e=>e.kind==='user_text'),'export.evidence','user caption needs user_text evidence');
  }
