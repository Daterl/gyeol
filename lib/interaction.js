...
import { ContractError, sameStructure, validateCaptionSlot, validateExport, validateFeed, validateInputIds, validatePhoto, validatePhotoPlan, validatePhotoRefs, validateProfile } from './contracts.js';
...
const photoList = (photos, min = 0, max = 20) => {
  requireValue(Array.isArray(photos) && photos.length >= min && photos.length <= max,'photos','invalid photo count');
...
// ADR-0008 production boundary. Legacy identity requests remain internal compatibility only.
export function validateCurationRequest(value) {
  fields(value,['schema_version','session_id','profile_url','profile_snapshot_id','photos','prompt'],'curation');
  version(value); text(value.session_id,'session_id',96);
  reference(value.profile_url); text(value.profile_snapshot_id,'profile_snapshot_id',4096);
  requireValue(Array.isArray(value.photos) && value.photos.length<=15,'photos','expected 3–15 photos');
  validateInputIds(photoList(value.photos,3));
  if (value.prompt!==undefined) requireValue(typeof value.prompt==='string' && value.prompt.length<=2000,'prompt','expected string up to 2000 characters');
  return value;
}
...
export function validateOrderRequest(value) {
  ...
  const ids=photoList(value.photos,3,15); validateInputIds(ids); validateIdentity(value.identity);
...
export const OMISSION_RULE = 'gyeol.omit.disclosure';

// 비움이 0개인 것도 판단의 결과다. 개수를 조작하지 않고, 최종 결과를 세어 그 사실만 응답에 적는다 (#80).
export const discloseOmission = slots => {
  const total=slots.length;
  const omitted=slots.filter(slot=>slot.caption_state==='omitted').length;
  return {
    omitted, total,
...
// 비움 개수는 추론이 아니라 응답 자체를 센 값이다. 세지 않고 지어낸 값은 여기서 거부된다 (#80).
export function validateOmission(omission, slots) {
  fields(omission,['omitted','total','note_key','note','evidence'],'omission');
  const omitted=slots.filter(s=>s.caption_state==='omitted').length;
  requireValue(omission.omitted===omitted,'omission.omitted','must count the returned slots');
  requireValue(omission.total===slots.length,'omission.total','must count the returned slots');
  requireValue(omission.note_key===(omitted===0?'omission.none':'omission.some'),'omission.note_key','disagrees with the counted omissions');
  const expected=discloseOmission(slots);
  requireValue(omission.note===expected.note,'omission.note','must describe the counted slots');
  requireValue(sameStructure(omission.evidence,expected.evidence),'omission.evidence','must contain only the canonical counting evidence');
  return omission;
}
...
export function validateGenerateResponse(value, request) {
  ...
  if (request.mode==='all') {
    fields(value,['output','omission'],'generation'); validateExport(value.output,request.feed,ids);
    requireValue(value.output.slots.every(s=>s.caption_state!=='user'),'generation','server cannot invent a user edit');
    if (value.omission!==undefined) validateOmission(value.omission,value.output.slots);
  } else {
