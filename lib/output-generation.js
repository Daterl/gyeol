...
// filled 는 완성된 캡션이 아니라 사용자가 자기 말로 완성할 단서다 (P1).
...
const validateHint = text => {
  const match=HINT_SHAPE.exec(typeof text==='string'?text:'');
  if (!match) throw new Error('filled text is not a short hint');
  ...
  // 소재 문구가 관측 사실을 옳게 옮겼는지는 코드로 판정하지 않는다. 실측 12회에서 거부된 41건 중
  // 대부분이 '진주 형태의 흰 목걸이'->'진주 목걸이'처럼 사실은 맞고 낱말만 바꾼 경우였다 (#101 리뷰).
  // 출처 추적은 아래 uploaded_photo note 의 원문 일치가 맡고, 의미 품질은 사람 인수 게이트에 남긴다.
  for (const seed of seeds) if (!seed.trim()) throw new Error('Empty hint seed');
};
...
const validatePublicOutput = (observation, input) => {
  ...
  for (const slot of slots) {
    ...
    if (slot.caption_state==='filled') validateHint(slot.text);
  }
