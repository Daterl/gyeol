...
const internalOutputPatterns = [
  /\b(?:rationale|narrative_role|caption_inputs|describable_facts|adjacent_overlap|is_visual_peak|applied_profile|target_profile_id|current_profile_id|photo_plan_id|caption_state|omit_reason|photo_id|empty_caption_ratio|caption_coverage|caption_len|hue_mean|sat_mean|bright_mean)\b/i,
  /(?:색\s*거리|측정\s*색|지향\s*방향|서사\s*규칙|앞자리\s*사진|남은\s*사진\s*중)/,
  /\bR[1-4]\b/i,
  /(?:밝기|채도|색상각)\s*(?:값|점수)?\s*[=:]?\s*0(?:\.\d+)?/i,
  /\b(?:hue|sat|bright|saturation|brightness)\s*[=:]\s*-?\d+(?:\.\d+)?\b/i,
  /점수가\s*(?:가장\s*)?(?:높|낮)/,
...
const validatePublicText = (value, facts=[]) => {
  ...
  if (internalOutputPatterns.some(pattern=>pattern.test(value))) throw new Error('Internal ordering detail in public output');
  for (const term of value.match(/밝기|채도/g)??[]) {
    if (!facts.some(fact=>fact.includes(term))) throw new Error('Unobserved measurement term in public output');
  }
};
