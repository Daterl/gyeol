...
const stabilizeOmission = (observation, input) => {
  ...
  const targetLanguage=input.context.target.kind==='photo_plan'?null:input.context.target.language;
  const ratios=[targetLanguage?.empty_caption_ratio?.value,input.context.current.language?.empty_caption_ratio?.value]
    .filter(value=>value!==undefined);
  if (!appliedLanguage || ratios.includes(0) || !ratios.some(value=>value>0)
    || Math.max(...ratios)*input.feed.slots.length<1
    || targetLanguage?.caption_len?.value?.p50>=90 || appliedLanguage.caption_len?.value?.p50>=90) return observation;
