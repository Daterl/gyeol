...
// 다섯 자 이상 이어진 발췌만 본다 — 짧은 낱말 겹침은 발췌가 아니다.
const OVERLAY_LIFT_MIN = 5;
const validateTitleOverlay = (title, photos) => {
  const value=typeof title==='string'?title:'';
  for (const photo of photos ?? []) {
    const overlay=typeof photo?.text_in_image==='string'?photo.text_in_image:'';
    for (let i=0;i+OVERLAY_LIFT_MIN<=overlay.length;i++) {
      if (value.includes(overlay.slice(i,i+OVERLAY_LIFT_MIN))) throw new Error('Title lifts a phrase from one photo on-image text');
    }
  }
};
