...
const PATH = /^profile-cache\/v1\/[a-f0-9]{64}\.json$/;
...
export function createPrivateBlobStorage({ token = process.env.BLOB_READ_WRITE_TOKEN, fetchImpl = fetch, timeoutMs = 10000 } = {}) {
  ...
  const path = value => {
    if (!PATH.test(value)) throw new ProfileStorageError('INVALID_STORAGE_PATH');
    return value;
  };
  ...
  return {
    ...
    async write(key, value, { ifMatch } = {}) {
      ...
      const response = await request(`${API}/?${new URLSearchParams({ pathname: path(key) })}`, {
        method: 'PUT', body: JSON.stringify(value), headers: {
          'content-type': 'application/json', 'x-content-type': 'application/json', 'x-access': 'private',
    ...
    async list({ cursor } = {}) {
      const query = new URLSearchParams({ prefix: PROFILE_CACHE_PREFIX, limit: '1000', ...(cursor ? { cursor } : {}) });
