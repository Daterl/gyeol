...
export class ProfileStorageError extends Error {
  constructor(code = 'STORAGE_ERROR', retryAfter = 0) {
    super(code); this.name = 'ProfileStorageError'; this.code = code; this.retryAfter = retryAfter;
  }
}
...
export function createPrivateBlobStorage({ token = process.env.BLOB_READ_WRITE_TOKEN, fetchImpl = fetch, timeoutMs = 10000, namespace = 'profile-cache' } = {}) {
  ...
  const rateLimitError = response => new ProfileStorageError('RATE_LIMITED',
    /^\d+$/.test(response.headers.get('retry-after') ?? '') ? Number(response.headers.get('retry-after')) : 0);
  return {
    async read(key) {
      ...
      if (!response.ok) {
        ...
        if (body?.error?.code === 'rate_limited') throw rateLimitError(response);
