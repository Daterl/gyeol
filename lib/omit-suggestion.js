...
// 응답의 빼기 권고를 만든 규칙 집합의 번호. 오름차순이고, 새 규칙은 뒤에만 붙는다.
//   1 — 동일 바이트 중복만 (#88). 이 PR 이전에 배포된 브라우저 번들이 아는 전부다.
//   2 — 1 + 화면 배치 유사 (#97).
// 왜 번호가 필요한가: 브라우저는 서버 응답의 omit_suggestion 을 **자기 코드로 다시 계산해** 대조한다
// (lib/contracts.js). 그래서 배포 직후 열려 있던 옛 번들에 규칙 2 응답을 주면, 그 번들은 규칙 1 로
// 계산한 결과와 달라 정상 응답을 INVALID_RESPONSE 로 버린다. 새 선택 필드를 더한 것만으로는 호환되지 않는다.
// 해결: 클라이언트가 자기가 아는 최대 규칙 번호를 헤더로 알리고, 서버는 그 이하로만 답한다.
// 헤더가 없으면 옛 번들이므로 규칙 1 로 답한다 — 옛 번들에게 이 기능은 없는 것이 되고, 그것이 정상이다(P3).
export const OMIT_RULES = [1, 2];
export const LATEST_OMIT_RULES = OMIT_RULES[OMIT_RULES.length - 1];
export const OMIT_RULES_HEADER = 'X-Gyeol-Omit-Rules';

// 서버가 이 요청에 대해 쓸 규칙 번호. 클라이언트가 요구한 것보다 높게 답하지 않는다.
// 서버보다 새 클라이언트(롤백 창)는 서버가 아는 최대치로 내려 받고, 그 번들은 자기보다 낮은 규칙의
// 응답도 받아들이도록 아래 validate 가 열려 있으므로 양방향 모두 끊기지 않는다.
export function negotiatedOmitRules(request) {
  const wanted = Number(request?.headers?.get?.(OMIT_RULES_HEADER));
  const usable = OMIT_RULES.filter(rules => rules <= wanted);
  return usable.length ? Math.max(...usable) : OMIT_RULES[0];
}
...
function decide(photos, byId, rules) {
  ...
  for (const photo of [...photos].sort((a, b) => a.input_index - b.input_index)) {
    ...
    const similar = rules >= 2 ? nearestRetained(photo, retained) : null;
...
export function withOmitSuggestions(feed, photos, { rules = LATEST_OMIT_RULES } = {}) {
  ...
  const decisions = decide(photos, byId, rules);
  let similarCount = 0;
