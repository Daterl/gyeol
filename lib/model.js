// Shared structured transport; built-in fetch, one deadline, no implicit fallback.
import { setTimeout as delay } from 'node:timers/promises';
import config from '../config/models.json' with { type: 'json' };
export const DEFAULT_MODEL = config.vision_model;
// Output generation is text-only and runs under a serverless deadline, so it gets its own model.
export const OUTPUT_MODEL = config.output_model ?? config.vision_model;
...
export function modelRoute(apiKey = process.env.ANTHROPIC_API_KEY) {
  if (typeof apiKey !== 'string' || !apiKey.trim()) return { source: 'heuristic', reason: 'missing_api_key' };
  return process.env.GYEOL_MODEL_PROVIDER_ENABLED === '1'
    ? { source: 'vision_model', reason: 'api_key_present' }
    : { source: 'heuristic', reason: 'provider_disabled' };
}

export async function analyzeWithModel({ bytes, mediaType, ...options }) {
  if (modelRoute(options.apiKey).source !== 'vision_model') throw new ModelError('MODEL_KEY_MISSING', 'ANTHROPIC_API_KEY is missing; no model request was made.');
  if (!IMAGE_TYPES.includes(mediaType)) throw new ModelError('MODEL_MEDIA_UNSUPPORTED', 'Model accepts JPEG, PNG, WebP or GIF only.');
  return requestStructuredModel({ ...options, requireImage: true, content: [{ type: 'image', source: {
    type: 'base64', media_type: mediaType, data: Buffer.from(bytes).toString('base64')
  } }] });
}

export async function requestStructuredModel({
  content, prompt, schema, apiKey = process.env.ANTHROPIC_API_KEY,
  model = process.env.GYEOL_VISION_MODEL || DEFAULT_MODEL, requireImage = false,
  timeoutMs = MODEL_TIMEOUT_MS, maxTokens = 4096, fetchImpl = globalThis.fetch,
      ...
      try {
        response = await fetchImpl(`${API}${path}`, {
          ...
          headers: {
            ...
            // 워크스페이스에 묶이지 않은 키는 이 헤더가 없으면 400 을 돌려준다.
  ...
  try {
    return await Promise.race([deadline, (async () => {
      ...
      if (!info || typeof info.id !== 'string' || !info.id || (requireImage && info.capabilities?.image_input?.supported === false)
      ...
      const body = await request('/messages', {
        model: info.id, max_tokens: maxTokens,
        system: prompt,
        output_config: { format: { type: 'json_schema', schema } },
        messages: [{ role: 'user', content }]
      });
      if (!body || body.stop_reason !== 'end_turn') throw new ModelError('MODEL_INCOMPLETE', 'Model refused or did not complete its response.');
      const blocks = body.content;
      if (!Array.isArray(blocks)) throw new ModelError('MODEL_JSON', 'Expected a content block array.');
      // thinking 이 켜진 모델은 [thinking, text] 처럼 블록을 여러 개 준다.
      // 우리가 읽을 것은 text 블록 하나뿐이므로 그것만 고른다.
      const textBlocks = blocks.filter((b) => b && b.type === 'text');
      if (textBlocks.length !== 1) {
        throw new ModelError('MODEL_JSON', `Expected exactly one text block, got ${textBlocks.length} of ${blocks.length}.`);
      }
      let observation;
      try { observation = JSON.parse(textBlocks[0].text); }
