...
import {LATEST_OMIT_RULES,negotiatedOmitRules,withOmitSuggestions} from './omit-suggestion.js';
...
function preserveOrder(photos, current, target, sessionId) {
  ...
  const slots=photos.map((photo,index)=>{
    ...
    // rulePlaced:false — 이 갈래는 R2·R3 가 아니라 입력 순서가 자리를 정했다. 마지막 자리가 "남은 사진 중
    // 가장 어두워"서 거기 있는 것이 아니므로 그 선택 근거를 말하지 않는다(lib/curation-voice.js 주석).
    const voice=placementVoice(photo,photos[index-1],role,photos[index+1],{position:index+1,count:photos.length,rulePlaced:false});
...
export async function buildFeed(input,{omitRules=LATEST_OMIT_RULES}={}) {
  ...
  const composed=target.kind==='photo_plan' && !hasOrderingBasis(photos)
  ...
  // 협상된 규칙으로 빼기 권고를 다시 정한다. withOmitSuggestions 는 기존 omit_suggestion 을 읽지 않고
  // PhotoAnalysis 에서 통째로 다시 계산하므로, 규칙 1 이면 옛 서버와 바이트 단위로 같은 응답이 된다.
  const feed=withOmitSuggestions(composed,photos,{rules:omitRules});
...
export async function handleLegacyFeed(request,{receiptSecret=process.env.GYEOL_ANALYSIS_RECEIPT_SECRET}={}) {
  try {
    ...
    return Response.json(await buildFeed(input,{omitRules:negotiatedOmitRules(request)}),{headers});
  }
  catch(error) { return failure(error); }
}
...
export async function handleFeed(request, options = {}) {
  try {
    const input=await readJsonRequest(request);
    return Response.json(await buildCuration(input,{omitRules:negotiatedOmitRules(request),...options}),{headers});
  } catch(error) { return failure(error); }
}
