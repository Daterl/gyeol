...
function preserveOrder(photos, current, target, sessionId) {
  ...
  const slots=photos.map((photo,index)=>{
    ...
    return {photo_id:photo.photo_id,position:index+1,narrative_role:index===0?'opener':index===photos.length-1?'closer':'sustain',
      rationale:{value:'올린 사진들의 밝기와 색이 서로 거의 같아 순서를 바꿀 근거가 없어요. 올린 순서를 그대로 두었어요.',confidence:1,evidence:[{kind:'uploaded_photo',ref:photo.photo_id,note:`측정값 — 밝기 ${photo.color.bright_mean} · 채도 ${photo.color.sat_mean}`},{kind:'rule',ref:'order.no_measured_difference',note:`묶음의 밝기 범위·채도 범위·최대 색 거리가 모두 R1 동점 밴드(${TIE_BAND}) 안이라 측정값이 사진들을 가르지 못했다. 가르지 못한 값으로 자리를 바꾸지 않는다`}]},
