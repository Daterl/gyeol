...
export function createPrivateShareBlobStore({
  ...
  return {
    ...
    // Exactly one write mode. `revision` replaces `ifMatch` where the caller guards the write
    // with the manifest's own revision instead of a Blob ETag (#210).
    async put(pathname, body, { contentType: type, ifMatch, ifNoneMatch, revision } = {}) {
      const modes =
        (ifMatch !== undefined ? 1 : 0) + (ifNoneMatch ? 1 : 0) + (revision ? 1 : 0);
      if (modes !== 1) throw new ShareError('INVALID_STORAGE_WRITE');
      try {
        const result = await client.put(path(pathname), Buffer.from(body), {
          ...options,
          access: 'private',
          addRandomSuffix: false,
          allowOverwrite: ifMatch !== undefined || revision === true,
