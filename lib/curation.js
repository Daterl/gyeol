...
import {LATEST_OMIT_RULES,withOmitSuggestions} from './omit-suggestion.js';
...
export async function buildCuration(input,{
  ...
  // 빼기 권고는 호출한 번들이 아는 규칙까지만 낸다 (lib/omit-suggestion.js OMIT_RULES).
  const feed=withOmitSuggestions(composeFeed({photoAnalyses:photos,targetProfile:target,currentProfile:current,currentPhotoAnalyses:[],sessionId:input.session_id}),photos,{rules:omitRules});
  ...
  return {...result,curation:{
    ...
    profile:{display:{username:snapshot.handle,
      ...(snapshot.profile_display?.name_source==='apify.ownerFullName' && validProfileDisplayName(snapshot.profile_display.display_name)
        ?{display_name:snapshot.profile_display.display_name.trim(),name_source:'apify.ownerFullName'}:{})},snapshot_id:snapshot.snapshot_id,source_url:record.source_url,collected_at:record.collected_at,
