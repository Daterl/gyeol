...
import {authorizeProfileBrowser,profileErrorResponse} from './profile-session.js';
...
export async function handleProfileConnection(request,{accessKey=process.env.APIFY_INGEST_ACCESS_KEY,cache,limiter,browser}={}) {
  ...
  try {
    ...
    if(actual!==null) {
      ...
      // Bearer is for non-browser automation; never allow cross-origin browser use.
      if(request.headers.has('origin') || request.headers.has('sec-fetch-site')) throw new RequestError('FORBIDDEN',403,'자동화 요청 경로를 확인해 주세요.');
    } else {
      if(!request.headers.has('cookie')) throw new RequestError('UNAUTHORIZED',401,'프로필 연결을 다시 시작해 주세요.');
      sessionId=authorizeProfileBrowser(request,browser);
    }
    const input=await readJsonRequest(request,8192);validateRequest(input);
    await (limiter??createProfileRequestLimit()).take(input.action,sessionId);
