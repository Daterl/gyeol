// JPEG DC-only reader using Node built-ins. The heuristic reports measured pixels
// without adding another image decoder to the Next.js application.
//
...
// without an IDCT and without chroma upsampling.
//
// The AC coefficients are not USED, but in a baseline scan they must still be
// CONSUMED: baseline interleaves DC+AC per block in one entropy stream, so the 63 AC
// coefficients sit between this block's DC and the next block's DC. Skipping them
// without decoding leaves the bit reader mid-block and every later DC is read from
// the wrong offset — which returned wrong colors rather than failing (review-codex.md
// H1). A progressive DC-first scan carries no AC, so there is nothing to consume.
//
...
// ponytail: bounded fixture/heuristic decoder; larger originals need the #24 upload policy.
...
// Walk one block's 63 AC coefficients so the reader lands on the next block's DC.
// Run-length/size coded: s===0 means EOB (rest of block is zero) unless r===15, the
// ZRL escape for 16 zeroes. Values are decoded and discarded — only the offset matters.
function consumeAcBlock(reader, table) {
  for (let k = 1; k < 64;) {
    const rs = decodeHuffman(reader, table);
    if (rs < 0) return false;
    const r = rs >> 4, s = rs & 15;
    if (s === 0) {
      if (r !== 15) return true;                              // EOB
      k += 16; continue;                                      // ZRL
    }
    k += r + 1;
    reader.receive(s);
  ...
  for (let mcu = 0; mcu < total; mcu++) {
    ...
    for (let i = 0; i < scan.length; i++) {
      ...
      for (let v = 0; v < c.v; v++) for (let h = 0; h < c.h; h++) {
        ...
        if (!frame.progressive && !consumeAcBlock(reader, scan[i].acTable)) { reader.eos = true; break; }
      }
    }
    if (reader.eos) break;
    decoded = mcu + 1;
  ...
  while (pos < bytes.length - 1) {
    ...
    if (marker === 0xDB) {                                     // DQT
      ...
      while (p < segEnd) {
        ...
        if (precision > 1 || id > 3 || p + (precision ? 128 : 64) > segEnd) return null;
      ...
      while (p < segEnd) {
        ...
        if (kind > 1 || id > 3 || p + 16 > segEnd) return null;
        const counts = Array.from(bytes.subarray(p, p + 16)); p += 16;
        const n = counts.reduce((a, b) => a + b, 0);
        if (!n || n > 256 || p + n > segEnd) return null;
      ...
      if (length < 8 || ![1, 3].includes(bytes[seg + 5]) || length !== 8 + 3 * bytes[seg + 5]) return null;
      ...
      if (frame.width * frame.height > MAX_PIXELS) return null;
      if (frame.components.some(c => c.h < 1 || c.h > 4 || c.v < 1 || c.v > 4 || c.tq > 3)) return null;
      if (new Set(frame.components.map(c => c.id)).size !== frame.components.length
        || frame.components.reduce((sum, c) => sum + c.h * c.v, 0) > 10) return null;
      ...
      if (!frame || count !== frame.components.length || length !== 6 + 2 * count) return null;
      const scan = [];
      for (let i = 0; i < count; i++) {
        const id = bytes[seg + 1 + i * 2], tables = bytes[seg + 2 + i * 2];
        const component = frame?.components.find(c => c.id === id);
        if (!component || scan.some(s => s.component === component)) return null;
        scan.push({ component, dcTable: huffDC[tables >> 4], acTable: huffAC[tables & 15] });
      }
      const ss = bytes[seg + 1 + count * 2], ah = bytes[seg + 3 + count * 2] >> 4, al = bytes[seg + 3 + count * 2] & 15;
      // Baseline cannot be decoded without the AC tables (they are what makes the
      // block boundaries findable), so a scan missing one is not usable.
      const usable = frame && scan.length === frame.components.length
        && scan.every(s => s.dcTable && (frame.progressive || s.acTable))
