...
import { validateFeed, validatePhoto, validateProfile } from './contracts.js';

const names = ['ordered_feed', 'photo_analysis', 'target_profile', 'current_profile'];
// Next.js runs at the project root; traced fixtures retain this path in Vercel.
const read = async name => JSON.parse(await readFile(join(process.cwd(), 'fixtures', `${name}.sample.json`), 'utf8'));
