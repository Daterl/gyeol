...
async function modelImage(bytes, mediaType) {
  if (!['image/jpeg','image/png','image/webp'].includes(mediaType)) return {bytes,mediaType};
  try {
    ...
    return {bytes:await image.rotate().resize({width:MODEL_IMAGE_SIDE,height:MODEL_IMAGE_SIDE,fit:'inside',withoutEnlargement:true}).jpeg({quality:80}).toBuffer(),mediaType:'image/jpeg'};
    ...
    if (route.source === 'vision_model') {
      ...
      try {
        ...
        const observed = validateObservation(result.observation, { photoId, fileRef, inputIndex }, result.model,
          resolvedType==='image/jpeg' ? measurement?.color : null);
        observation = {
          ...observed,
