...
async function measureRaster(bytes, mediaType) {
  ...
  try {
    ...
    const encoded=await raster.flatten({background:'#ffffff'}).resize({width:512,height:512,fit:'inside',withoutEnlargement:true}).jpeg({quality:90}).toBuffer();
...
function measuredFacts(m) {
  const facts = [];
  if (m.source==='raster-thumbnail-jpeg-dc') facts.push('색 수치는 긴 변 최대 512px JPEG 변환본의 근사 측정');
