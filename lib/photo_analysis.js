...
import { exifOrientation, orientSignature, structureSignature } from './photo_signature.js';
...
export function measureJpeg(bytes) {
  ...
  // 서명은 저장 픽셀이 아니라 화면에 보이는 방향으로 잰다 — EXIF Orientation 만 다른 두 파일은
  // 같은 사진이고, 물리 회전된 재인코딩본과도 같은 배치다(#182 리뷰 Major 2).
  const cells = structureSignature(luma);
  const structure_signature = cells && orientSignature(cells, exifOrientation(bytes));
  return {
    source: 'jpeg-dc',
...
async function measureRaster(bytes, mediaType) {
  ...
  try {
    ...
    const encoded=await raster.rotate().flatten({background:'#ffffff'}).resize({width:MODEL_IMAGE_SIDE,height:MODEL_IMAGE_SIDE,fit:'inside',withoutEnlargement:true}).jpeg({quality:90}).toBuffer();
...
function heuristicObservation(m) {
  return {
    ...
    // 측정에서 구조가 관측된 사진만 서명을 싣는다. 단색·저대비 카드는 필드 자체가 없다.
    ...(m.structure_signature ? { structure_signature: m.structure_signature } : {}),
    ...
    if (route.source === 'vision_model') {
      ...
      try {
        ...
        observation = {
          ...
          // 구조 서명은 픽셀 측정에서만 나온다. 모델은 이 필드를 낼 수 없다(OBSERVATION_SCHEMA 밖).
          ...(measurement?.structure_signature ? { structure_signature: measurement.structure_signature } : {}),
