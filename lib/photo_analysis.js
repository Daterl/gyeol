...
const unescapeXml = value => value.replace(/&(#x[0-9a-fA-F]+|#\d+|[a-zA-Z]+);/g, (whole, entity) =>
  entity[0] === '#'
    ? String.fromCodePoint(Number(entity[1] === 'x' ? `0x${entity.slice(2)}` : entity.slice(1)))
    : (XML_ENTITIES[entity.toLowerCase()] ?? whole));
...
export function measureSvg(bytes) {
  ...
  const text_in_image = textAttrs !== undefined && !HIDDEN.test(textAttrs)
    ? (unescapeXml(textBody).trim() || null)
    : null;
  return {
    source: 'svg-card',
...
function heuristicObservation(m) {
  return {
    ...
    model: m.source === 'svg-card' ? 'heuristic-svg-card@1' : 'heuristic-jpeg-dc@1',
...
// Only the fields the observation schema declares are taken from a model response.
// Which photo this is (schema_version/photo_id/file_ref/input_index) is the caller's
// fact, and a response carrying those fields was able to relabel the analysis as
// another photo's (review-codex.md M2) — the same shape of hole as the foundation
// review's H1/H2: a field was accepted without being resolved against the real input.
const pickObservation = raw => Object.fromEntries(
  OBSERVATION_FIELDS.filter(field => field in raw).map(field => [field, raw[field]]));
...
function stampAnalysis(observation, { photoId, fileRef, inputIndex }, extraFlags = []) {
  const { measurement, usage, originPhotoId, ...facts } = observation;
  return {
...
export async function analyzePhoto({
  ...
  if (fromCache) counters.cacheHits++;
    ...
    if (apiKey) {
      ...
      try {
        ...
        const observed = pickObservation(result.observation ?? {});
        observation = {
          ...observed,
        ...
        // The contract check belongs here — inside the fallback and before the cache
        // write. It used to run after both, so a contract-violating response became a
        // 500 instead of a heuristic, counted as zero model failures, and stayed in
        // the cache: every later request for the same bytes threw the same error
        // without retrying the model (review-codex.md H2).
  ...
  const duplicate = fromCache && cached.originPhotoId && cached.originPhotoId !== photoId
    ? [`duplicate_of:${cached.originPhotoId}`] : [];
  const analysis = stampAnalysis(cached, { photoId, fileRef, inputIndex }, duplicate);
  validatePhoto(analysis);
  return { analysis, fromCache, measurement: cached.measurement, usage: cached.usage, digest };
}
