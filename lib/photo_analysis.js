...
// Validate before selecting fields or replacing model color with measured pixels.
function validateObservation(raw, identity, model) {
  ...
  validatePhoto({ ...raw, schema_version: '1.0', photo_id: identity.photoId, file_ref: identity.fileRef,
  ...
  if (raw.quality_flags.some(flag => !['dark', 'blurry'].includes(flag))) throw new ContractError('observation.quality_flags', 'model cannot assert duplicate identity');
  return raw;
}
...
export async function analyzePhoto({
  ...
  if (fromCache) counters.cacheHits++;
    ...
    if (route.source === 'vision_model') {
      counters.modelCalls++;
      try {
        const result = await client({ bytes: buffer, mediaType: resolvedType, prompt, schema: OBSERVATION_SCHEMA, model, apiKey, timeoutMs });
    ...
    observations.set(cacheKey, structuredClone({ ...observation, originPhotoId: photoId }));
  }
