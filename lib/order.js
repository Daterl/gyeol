...
//    그리고 휴리스틱 경로에서는 그마저도 관측이 아니라 상수이므로 근거 문장에서 뺀다 (#69, measured 위 주석)
...
// 구도는 모델이 본 사진에서만 관측된다. 휴리스틱 경로의 composition 은 항상 'full_frame' 인 상수이고
// (lib/photo_analysis.js 는 그 경로에서 negative_space 를 낼 수 없다, test/photo_analysis.test.js
// "heuristic must not claim negative space"), 상수를 관측처럼 말하면 근거 없는 주장이 된다.
// 점수(WEIGHT)에서는 그대로 쓴다 — 휴리스틱에서는 전 사진 공통이라 순위를 흔들지 않고, 점수는 사용자에게
// 보이는 문장이 아니다. 사용자에게 보이는 문장에서만 뺀다.
const observedComposition = photo => photo.source === 'vision_model';

const measured = photo =>
  `측정값 — 밝기 ${round(photo.bright)} · 채도 ${round(photo.sat)}`
  + (observedComposition(photo) ? ` · ${photo.flat ? '한 색이 넓게 깔림' : '한 색이 넓게 깔리지 않음'}` : '')
...
function rationale({ photo, role, rule, position, previous, direction, count, bonus, score, tie }) {
  ...
  const flatWord = observedComposition(photo) ? (photo.flat ? '한 색이 넓게 깔린 화면' : '한 색이 넓게 깔리지 않은 화면') : null;
  const text = {
