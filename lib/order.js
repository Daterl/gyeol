...
function rationale({ photo, role, rule, position, previous, direction, count, bonus, score, tie }) {
  ...
  const voice = placementVoice(photo.analysis, previous?.analysis, role);
  evidence.push({kind: 'rule', ref: `order.${rule}.decision`, note: text}, ...voice.evidence);
  // 첫 자리는 지향이 순서에 개입하는 유일한 자리다(R2·R3·R4 는 지향을 읽지 않는다). 그래서 지향이 없을 때만
  // 첫 자리 문장이 그 사실을 말한다 — 두 경로의 근거가 서로 다른 것을 말해야 한다(#127 spec 2-3).
  const value = role === 'opener' && direction === PHOTO_ONLY_DIRECTION ? PHOTO_ONLY_OPENER : voice.value;
