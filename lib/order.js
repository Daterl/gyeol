...
function resolveDirection(target) {
  ...
  return { kind: 'none', confidence: 1, evidence: [], note: '지향에 구성비도 문구도 없어 기본 규칙만 썼다' };
}
...
function rationale({ photo, role, rule, position, previous, direction, count, bonus }) {
  const bright = round(photo.bright), sat = round(photo.sat);
  const flatWord = photo.flat ? '한 색이 넓게 깔린 화면' : '한 색이 넓게 깔리지 않은 화면';
  const text = {
    opener: direction.kind === 'none'
      ? `밝기 ${bright} · ${flatWord}이라 첫 자리 기본 규칙에 입력 ${count}장 중 가장 잘 맞아 1번에 뒀다`
      : `밝기 ${bright} · 채도 ${sat} · ${flatWord}이라 지향 방향(${DIRECTION_WORD[direction.kind]}) 점수가 입력 ${count}장 중 가장 높아 1번에 뒀다`,
    sustain: previous
  ...
  return { value: bonus ? `${text} 캐러셀 여는 사진 경향(${bonus.note})도 같은 방향이다.` : text, confidence: role === 'opener' ? direction.confidence : 1, evidence };
}
...
export function orderFeed({ photoAnalyses, targetProfile, currentProfile, sessionId, feedId, now } = {}) {
  ...
  placed[1] = { photo: take(photo => WEIGHT[direction.kind](photo) + (bonuses.get(photo.id) ? OPENER_BONUS : 0)), role: 'opener', rule: 'R1' };
  ...
  for (let position = 1; position <= count; position++) {
    ...
    slots.push({
      ...
      rationale: rationale({ photo, role, rule, position, previous, direction, count, bonus }),
      caption_inputs: {
