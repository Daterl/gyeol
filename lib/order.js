...
function rationale({ photo, role, rule, position, previous, next, direction, count, bonus, score, tie, missingProfilePalette }) {
  ...
  const voice = placementVoice(photo.analysis, previous?.analysis, role, next?.analysis);
  // This rationale already has the current photo's measurements. Retain that
  // original note and add only its verbatim observations, plus neighbor evidence.
