...
// 두 축의 캡션 길이를 합치는 규칙은 이 함수 하나뿐이다. lib/compose.js 가 이 값을 그대로 싣는다.
// 규칙을 두 곳에 두면 화면은 "29자를 제안합니다"라고 말하면서 순서는 292자로 잡힌다 — #99 가 잡아낸 상태가
// 정확히 그것이었다(applied_profile 은 292/290/29/527 로 갈라져 있는데 순서는 네 경우가 모두 같았다).
// 수식은 #13 이 쓰던 것을 옮겨 온 것이며 바꾸지 않았다. 새 기울기 상수를 도입하지 않는다.
// null 을 내는 조건이 이 함수의 본체다 — 관측되지 않은 축은 순서를 움직이지 못한다:
//   보정축이 없거나(present:false), 어느 한쪽이 캡션 길이를 재지 못했거나, 두 값이 같거나,
//   합성해도 지향 원값 그대로면 null 이다. 바뀐 게 없으면 바뀌었다고 말하지 않는다.
export function composeCaptionLen(targetProfile, currentProfile) {
  const target = targetProfile?.language?.caption_len;
  const current = currentProfile?.language?.caption_len;
  if (!currentProfile?.present || !target || !current) return null;
  ...
  if (resolved === t) return null;
  const evidence = clone([...target.evidence, ...current.evidence, {
  ...
  return {
    target: t, current: c, resolved, profileId: currentProfile.profile_id,
    claim: {
    ...
    delta: {
      ...
      rule: 'log_midpoint', note_key: 'caption_len_gap', evidence: clone(evidence)
    }
  };
}

// 보정축 근거를 슬롯에 실을 때 쓰는 한 건. 보정축 사진·게시물 근거를 그대로 넣으면 E10 이 거부한다
// (lib/contracts.js 는 feed.slots 안의 uploaded_photo 근거를 이번 회차 입력 사진으로만 허용한다).
// 원근거 전문은 applied_profile.deltas[0].evidence 에 그대로 실려 검증되므로 추적은 끊기지 않는다.
const currentAxisEvidence = correction => ({
  kind: 'aggregate', ref: correction.profileId,
  note: `보정축 프로필이 잰 캡션 길이 p50 ${correction.current}자 — 원근거는 applied_profile.deltas 의 caption_len_gap 에 있다`
});

// 지향 방향은 한 번만 정한다. 구조화된 수치가 있으면 그것이 이기고, 없으면 사용자가 쓴 말의 해석이며,
// 둘 다 없으면 방향 없이 기본 규칙만 쓴다. 애매하면 한쪽으로 밀지 않는다.
function resolveDirection(target, correction) {
  ...
  // 애매하므로 한쪽으로 밀지 않는다.
  // 여기가 보정축이 순서에 닿는 유일한 지점이다(#99). 지향이 잰 길이 대신 두 축 합성값을 읽는다 —
  // 화면의 간극 카드가 이미 그 값을 "제안합니다" 라고 말하고 있으므로, 순서만 다른 값을 쓰면 둘이 갈라진다.
  // 보정축이 없거나 캡션을 안 잰 사람이면 composeCaptionLen 이 null 이고, 이 분기는 수정 전과 완전히 같다.
  // 위 두 분기(구성비·문구)가 먼저 방향을 정했으면 보정축은 순서에 영향을 주지 않았으므로
  // 근거 문장에도 적지 않는다 — 안 쓴 값을 썼다고 말하는 것도 같은 크기의 거짓말이다.
  const length = target.language?.caption_len;
  if (length) {
    const p50 = correction ? correction.resolved : length.value.p50;
    const kind = p50 >= CAPTION_DENSE ? 'dense' : p50 <= CAPTION_QUIET ? 'quiet' : null;
    const evidence = correction ? [...clone(length.evidence), currentAxisEvidence(correction)] : clone(length.evidence);
    const read = correction
      ? `지향이 잰 캡션 길이 p50 ${correction.target}자와 보정축이 잰 ${correction.current}자를 합성한 ${p50}자`
      : `잰 캡션 길이 p50 ${p50}자`;
    if (kind) return {
      kind, confidence: 0.4, evidence, correction,
      note: `지향에 구성비도 문구도 없어 ${read}를 ${DIRECTION_WORD[kind]}으로 읽었다 (시각 의도가 아니라 언어축을 건너 읽은 해석이다)`
    };
    // 합성값이 두 기준값 사이로 떨어지면 방향이 없다. 한쪽으로 밀지 않는 것은 지향 단독일 때와 같은 판단이고
    // (바로 아래 return), 다만 그 결론에 보정축이 관여했으므로 근거는 여기서 달고 내려간다.
    if (correction) return {
      kind: 'none', confidence: 0.4, evidence, correction,
      note: `지향에 구성비도 문구도 없고, ${read}가 두 기준값(${CAPTION_QUIET}자·${CAPTION_DENSE}자) 사이라 방향 없이 기본 규칙만 썼다`
    };
  }
...
function rationale({ photo, role, rule, position, previous, direction, count, bonus, score, tie }) {
  ...
  // 보정축이 방향을 바꾼 회차에서는 그 사실이 사용자에게 보이는 문장에 있어야 한다(P2).
  // 두 관측값과 합성값, 그리고 "보정축이 없었다면 어떻게 읽었을지"를 같이 적어 무엇이 바뀌었는지 문장만으로 확인되게 한다.
  const correction = role === 'opener' ? direction.correction : null;
  const corrected = correction
    ? ` 이 방향은 지향이 잰 캡션 길이 ${correction.target}자를 보정축이 잰 ${correction.current}자와 합성한 ${correction.resolved}자로 정했다 — 보정축이 없었다면 ${correction.target}자로 읽었다.`
    : '';
  ...
  return { value: corrected ? `${text.replace(/\.$/, '')}.${corrected}` : text, confidence: role === 'opener' ? direction.confidence : 1, evidence };
}
...
export function orderFeed({ photoAnalyses, targetProfile, currentProfile, sessionId, feedId, now } = {}) {
  ...
  // 보정축이 순서에 닿는 지점(#99). lib/compose.js 의 간극 카드와 같은 함수를 써야
  // "순서는 29자로 잡았는데 화면은 292자" 가 되지 않는다 — 그게 #99 가 잡아낸 상태다.
  const correction = composeCaptionLen(targetProfile, currentProfile);
  const direction = resolveDirection(targetProfile, correction);
  ...
  const feed = {
    ...
    applied_profile: {
      ...
      // 여기에 correction.delta 를 싣지 않는 이유는 경계 때문이다 — 그 근거는 보정축 사진을 가리키는데
      // orderFeed 는 보정축 사진 ID 를 받지 않아 E10 을 만족시킬 수 없다(lib/compose.js 의 composeFeed 는
      // currentPhotoAnalyses 를 받아 검증까지 마친 뒤 이 블록을 통째로 덮어쓴다).
      // 순서가 합성값으로 정해졌다는 사실은 slots[].rationale 에 실린다.
