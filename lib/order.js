...
//    그리고 한 장이라도 휴리스틱이 섞인 배치에서는 관측이 아니라 상수이므로 점수에서도 문장에서도 뺀다
//    (#69 교차 리뷰 P2, attributes 위 주석)
...
import { ContractError, validateFeed, validateInputIds, validatePhoto, validatePhotoPlan, validateProfile } from './contracts.js';
...
const RULE_NOTE = {
  ...
  R4: '서사 규칙 R4 — 나머지는 앞자리 사진과 측정 색이 가장 먼 사진부터. 이미 놓은 사진과 측정 색이 같은 사진은 아직 놓지 않은 사진이 남아 있는 동안 뒤로 미룬다'
};
...
// 구도는 배치 전체가 모델 관측일 때만 순위와 근거에 쓴다. 한 장이라도 휴리스틱이 섞이면 그 장의
// composition 은 관측이 아니라 상수 'full_frame' 이고(lib/photo_analysis.js 는 그 경로에서 negative_space 를
// 낼 수 없다, test/photo_analysis.test.js "heuristic must not claim negative space"), 그 상수가 관측된 사진과의
// 순위를 가른다. 관측한 적 없는 값이 순위를 바꾸면 근거를 댈 수 없으므로 아예 쓰지 않는다 — flat 을 전 사진 0 으로
// 두면 WEIGHT 의 구도항이 배치 공통 상수가 되어 순위에서 사라지고, measured/rationale 문장에서도 빠진다.
// 전부 휴리스틱인 배치는 원래 전 사진 flat=0 이었으므로 이 처리 전후로 순서가 같다.
// #9(scale 상수) · #12(has_face 미관측) · #41 이 같은 상황에서 고른 판단과 같다: 관측 못 한 값은 버린다.
...
export function measuredColorOverlap(previous, current) {
  validatePhoto(previous); validatePhoto(current);
  const color = photo => ({bright:photo.color.bright_mean,sat:photo.color.sat_mean,hue:photo.color.hue_mean});
  return round(1-distance(color(previous),color(current)));
}
...
function resolveDirection(target) {
  ...
  return { kind: 'none', confidence: 1, evidence: [], note: length
    ? '지향에 구성비도 방향을 정할 문구도 없고 캡션 길이도 두 기준값 사이라 기본 규칙만 썼다'
    : '지향에 구성비도 방향을 정할 문구도 없고 캡션 길이 근거가 없어 기본 규칙만 썼다' };
}

// 사진만 올린 경로(PhotoPlan)의 방향. 지향이 없으므로 방향 점수는 기본 규칙(WEIGHT.none)이고, 그 사실을
// 문장으로 말한다. PhotoPlan 의 visual(palette·composition_mix·subjects)은 업로드한 사진 자신의 평균이지
// 사용자가 지향한 방향이 아니다 — 그것을 방향으로 읽으면 없는 지향을 지어내는 것이므로 읽지 않는다
// (#9 #12 #41 #69 이 같은 상황에서 고른 판단과 같다: 관측하지 않은 것은 쓰지 않는다).
const PHOTO_ONLY_DIRECTION = {
  kind: 'none', confidence: 1, evidence: [],
  note: '지향이 없어 올린 사진에서 관측된 밝기·채도·색 거리만으로 정했다 (지향 방향 점수는 쓰지 않았다)'
};
// 지향이 없을 때의 R1 규칙 문구. RULE_NOTE.R1 은 "지향 방향 점수가 가장 높은 사진"이라 이 경로에서는 사실이 아니다.
const PHOTO_ONLY_R1 = '서사 규칙 R1 — 지향이 없으면 첫 자리는 기본 규칙(관측된 밝기) 점수가 가장 높은 사진';
const PHOTO_ONLY_OPENER = '지향을 넣지 않아, 올린 사진에서 보이는 것만으로 이 사진을 첫 자리에 뒀어요.';

// 순서를 바꿀 근거가 묶음에 있는가. R2(밝기)·R3(채도)·R4(색 거리)가 쓰는 세 축 중 하나라도 묶음 안 차이가
// TIE_BAND 이상이면 참이다. 세 축 전부가 밴드 안이면 그 값들은 두 사진을 가르지 못한 것이고, 그때 자리를
// 바꾸면 "왜 이 순서인가"에 답할 수 없다 — R1 동점 밴드와 같은 이유로 같은 상수를 쓴다. 새 상수가 아니다.
// ponytail: 쌍별 거리 O(n²). 입력은 계약상 3~20장이므로 최적화하지 않는다.
export function hasOrderingBasis(analyses) {
  if (!Array.isArray(analyses) || analyses.length === 0) throw new ContractError('photoAnalyses', 'expected at least one PhotoAnalysis');
  analyses.forEach(validatePhoto);
  const colors = analyses.map(a => ({ bright: a.color.bright_mean, sat: a.color.sat_mean, hue: a.color.hue_mean }));
  const spread = key => Math.max(...colors.map(c => c[key])) - Math.min(...colors.map(c => c[key]));
  const farthest = Math.max(...colors.flatMap(a => colors.map(b => distance(a, b))));
  return spread('bright') >= TIE_BAND || spread('sat') >= TIE_BAND || farthest >= TIE_BAND;
}
...
const measured = photo =>
  `측정값 — 밝기 ${round(photo.bright)} · 채도 ${round(photo.sat)}`
  + (photo.observedComposition ? ` · ${photo.flat ? '한 색이 넓게 깔림' : '한 색이 넓게 깔리지 않음'}` : '')
...
function openerText({ bright, sat, flatWord, direction, count, bonus, score, tie }) {
  const look = flatWord ? `밝기 ${bright} · 채도 ${sat} · ${flatWord}` : `밝기 ${bright} · 채도 ${sat} 인 사진`;
  ...
  if (!bonus) return direction.kind === 'none'
    ? `${flatWord ? `밝기 ${bright} · ${flatWord}` : look}이라 첫 자리 기본 규칙에 입력 ${count}장 중 가장 잘 맞아 1번에 뒀다`
...
function rationale({ photo, role, rule, position, previous, next, direction, count, bonus, score, tie, missingProfilePalette }) {
  const bright = round(photo.bright), sat = round(photo.sat);
  const flatWord = photo.observedComposition ? (photo.flat ? '한 색이 넓게 깔린 화면' : '한 색이 넓게 깔리지 않은 화면') : null;
  const text = {
  ...
  evidence.push({
    ...
    note: (tie ? '서사 규칙 R1 — 보너스 포함 총점의 상위 밴드 안에서 지향 색에 가까운 사진'
      : direction === PHOTO_ONLY_DIRECTION && rule === 'R1' ? PHOTO_ONLY_R1 : RULE_NOTE[rule]) + (role === 'opener' ? `. ${direction.note}` : '')
  ...
  const voice = placementVoice(photo.analysis, previous?.analysis, role, next?.analysis, { position, count });
  // This rationale already carries the current photo's measurement note. Drop the voice's
  // duplicate for the same photo and keep the neighbor measurement and the rule notes.
  evidence.push({kind: 'rule', ref: `order.${rule}.decision`, note: text}, ...voice.evidence.filter(item =>
    item.kind !== 'uploaded_photo' || item.ref !== photo.id || photo.facts.includes(item.note)));
  // 첫 자리는 지향이 순서에 개입하는 유일한 자리다(R2·R3·R4 는 지향을 읽지 않는다). 그래서 지향이 없을 때만
  // 첫 자리 문장이 그 사실을 말한다 — 두 경로의 근거가 서로 다른 것을 말해야 한다(#127 spec 2-3).
  let value = role === 'opener' && direction === PHOTO_ONLY_DIRECTION ? `${PHOTO_ONLY_OPENER} ${voice.value}` : voice.value;
  if (role === 'opener' && missingProfilePalette) {
    // An unavailable observation is a scope disclosure, never a profile Claim.
    const note = '프로필의 색은 측정하지 않아 프로필 색에 맞춘 순서는 제안하지 않아요.';
    value += ` ${note}`;
    evidence.push({kind: 'rule', ref: 'order.profile_palette_unavailable', note});
  }
  return { value, confidence: role === 'opener' ? direction.confidence : 1, evidence };
}
...
export function orderFeed({ photoAnalyses, targetProfile, currentProfile, sessionId, feedId, now } = {}) {
  ...
  // 사진만 올린 경로는 TargetProfile 이 아니라 PhotoPlan 을 싣는다. 계약은 이미 두 축을 다 다룬다
  // (lib/contracts.js validateTargetAxis · validateFeed). 축 종류에 따라 검증만 갈라 부른다.
  ...
  const direction = plan ? PHOTO_ONLY_DIRECTION : resolveDirection(targetProfile);
  const bonuses = new Map(photos.map(photo => [photo.id, plan ? null : openerBonus(photo, targetProfile)]));
  ...
  // 지향이 없으면 "지향이 잰 색에 가까운 사진"이라고 말할 수 없고, PhotoPlan 의 palette 는 입력 평균이라
  // 순환 근거다. 타이브레이크를 켜지 않는다 — 동점은 입력 순서가 이긴다.
  const tiebreak = plan ? null : resolveTiebreak(targetProfile);
  ...
  //
  // 이미 놓은 사진과 **측정 색이 똑같은** 사진은 새 대비를 더하지 못한다. 그런데 R4 는 "앞자리에서
  // 가장 먼 사진"을 고르므로 그런 쌍 사이를 왕복하는 배치가 늘 최적이 된다 — 실사진 15장(고유 13장)에서
  // 4~7번이 A·B·A·B 로 나왔다(#109 품질 평가 ⑤). 같은 화면이 한 자리 건너 다시 나오는 배치다.
  // 그래서 아직 놓지 않은 화면이 남아 있는 동안에는 그 사진을 뒤로 미룬다. 판정은 이미 쓰고 있는
  // 측정 색 거리 하나이고 새 상수를 만들지 않는다. 측정 색이 같은 사진이 없는 묶음에서는 후보 집합이
  // 그대로라 순서가 한 자리도 바뀌지 않는다.
  for (let position = 2; position < count; position++) {
    if (placed[position]) continue;
    const previous = placed[position - 1].photo;
    // "이미 놓은" 은 **앞자리**다. placed 에는 turn·closer 가 뒷자리에 미리 들어가 있는데 그것까지 세면
    // 그 사진과 똑같은 화면이 후보에서 계속 밀려 마지막 남은 자리, 즉 자기 쌍 **바로 옆**에 붙는다
    // (밝기 최저 쌍이 중복일 때 재현된다). 피하려던 반복을 더 가깝게 만드는 배치라 앞자리만 센다.
    const shown = placed.slice(1, position).filter(Boolean).map(slot => slot.photo);
    const candidates = [...pool.values()];
    const unseen = candidates.filter(photo => !shown.some(other => distance(other, photo) === 0));
    const photo = pick(unseen.length ? unseen : candidates, candidate => distance(previous, candidate));
    pool.delete(photo.id);
    placed[position] = { photo, role: 'sustain', rule: 'R4' };
  }
  ...
  for (let position = 1; position <= count; position++) {
    ...
    slots.push({
      ...
      rationale: rationale({ photo, role, rule, position, previous, next: placed[position + 1]?.photo, direction, count, bonus, tie: position === 1 ? tie : null, missingProfilePalette: !plan && !tiebreak, score: { base: directionScore(photo), topByMeasure: photo.id === topByMeasureId } }),
      caption_inputs: {
        describable_facts: [...photo.facts],
        adjacent_overlap: previous ? measuredColorOverlap(previous.analysis,photo.analysis) : 0,
        is_visual_peak: photo.id === peakId
      }
    });
  }

  // 묶음 컨셉은 피드 단위 판단이다. 슬롯에 넣으면 사용자가 순서를 바꿀 때 원래 첫 사진을 따라 이동한다.
  const concept = bundleConcept(photoAnalyses);

  const digest = createHash('sha1').update(ids.join('|')).digest('hex').slice(0, 8);
  const feed = withOmitSuggestions({
    // PhotoPlan 을 실은 피드는 OrderedFeed 1.1 이다 (lib/contracts.js 가 강제한다). 새 규칙이 아니다.
    schema_version: plan ? '1.1' : '1.0',
    feed_id: feedId ?? `fd_${plan ? plan.plan_id : targetProfile.profile_id}_${digest}`,
    ...
    applied_profile: {
      ...
      // 사진만 올린 경로의 language 는 planFromPhotos 가 이미 null 로 낸다 — 여기서 만들지 않는다.
