...
function openerText({ bright, sat, flatWord, direction, count, bonus, score, tie }) {
  ...
  if (tie) return `${look}이다. ${label} 점수가 상위 ${tie.size}장을 ${TIE_BAND} 안에서 가르지 못해, ${tie.note}이라 1번에 뒀다 — 지향이 잰 색과의 거리 ${tie.distance}.`;
...
function rationale({ photo, role, rule, position, previous, direction, count, bonus, score, tie }) {
  ...
  evidence.push({
    ...
    note: RULE_NOTE[rule] + (role === 'opener' ? `. ${direction.note}` : '')
      + (tie ? `. 방향 점수가 상위 ${tie.size}장을 ${TIE_BAND} 안에서 가르지 못해 2순위 이하까지 내려가 지향이 잰 색으로 갈랐다` : '')
...
export function orderFeed({ photoAnalyses, targetProfile, currentProfile, sessionId, feedId, now } = {}) {
  ...
  // R1 은 방향 점수 1위를 고른다. 다만 1위와 그 아래가 TIE_BAND 안에 있으면 방향 점수는 두 사진을
