...
function rationale({ photo, role, rule, position, previous, direction, count, bonus, score, tie }) {
  ...
  return { value: voice.value, confidence: role === 'opener' ? direction.confidence : 1, evidence };
}
...
export function orderFeed({ photoAnalyses, targetProfile, currentProfile, sessionId, feedId, now } = {}) {
  ...
  slots[0].rationale.value = `${concept.value} ${slots[0].rationale.value}`;
  slots[0].rationale.evidence.push(...concept.evidence);
