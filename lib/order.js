...
function rationale({ photo, role, rule, position, previous, next, direction, count, bonus, score, tie }) {
  ...
  const value = role === 'opener' && direction === PHOTO_ONLY_DIRECTION ? `${PHOTO_ONLY_OPENER} ${voice.value}` : voice.value;
...
export function orderFeed({ photoAnalyses, targetProfile, currentProfile, sessionId, feedId, now } = {}) {
  ...
  for (let position = 1; position <= count; position++) {
    ...
    slots.push({
      ...
      rationale: rationale({ photo, role, rule, position, previous, next: placed[position + 1]?.photo, direction, count, bonus, tie: position === 1 ? tie : null, score: { base: directionScore(photo), topByMeasure: photo.id === topByMeasureId } }),
      caption_inputs: {
