...
test('profile HTTP rejects missing authorization and paid-start consent before cache access',async()=>{
  ...
  for(const authorization of ['', 'Bearer wrong']) {
    const response=await handleProfileConnection(request(input,authorization),{accessKey,cache});assert.equal(response.status,401);
  }
  for(const body of [{...input,confirmLive:false},{...input,confirmLive:undefined},{...input,snapshot:{status:'public'}},{...input,receipt:'client'},{...input,refresh:'yes'}]) {
    const response=await handleProfileConnection(request(body),{accessKey,cache});assert.equal(response.status,400);
  }
...
test('profile HTTP delegates connect and status, returning only safe cache connection metadata',async()=>{
  ...
  const pending=await handleProfileConnection(request({...input,refresh:true}),{accessKey,cache});
  ...
  const ready=await handleProfileConnection(request({schema_version:'1.0',action:'status',profile_url:url}),{accessKey,cache});
...
test('profile HTTP keeps private/unconfirmed outcomes explicit and hides storage errors',async()=>{
  for(const status of ['private','unconfirmed','timeout','not_found','expired']) {
    const response=await handleProfileConnection(request(input),{accessKey,cache:{connect:async()=>({status,refresh_required:status==='expired'})}});
  ...
  for(const code of ['STORAGE_ERROR','NOT_CONFIGURED']) {
    const response=await handleProfileConnection(request(input),{accessKey,cache:{connect:async()=>{throw Object.assign(new Error('secret details'),{code});}}});
    assert.equal(response.status,503);assert.ok(!JSON.stringify(await response.json()).includes('secret details'));
  }
  const invalid=await handleProfileConnection(request(input),{accessKey,cache:{connect:async()=>({status:'public',snapshot:{private:'payload'}})}});
