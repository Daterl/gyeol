# Instagram형 큐레이션·공유 디자인 계약 — 2026-09-18

담당 **diego.yoon** (`@jangwonyoon`) · 협업 **enzo.cho** (`@onejaejae`). 제품 흐름은 [ADR-0008](../adr/0008-public-profile-curation-and-sharing.md)과 [엄브렐라 #65](https://github.com/Daterl/gyeol/issues/65)를 따른다. 아래 규칙은 이 문서의 과거 종이 작업대 시안보다 우선하며, 제품 구현 완료를 뜻하지 않는다.

## 현재 Instagram 모바일 웹 관찰

2026-09-18 공개 프로필을 로그아웃 상태의 실제 모바일 웹 390×844px에서 확인했다. 개인 사진과 식별 가능한 캡처는 저장소에 넣지 않았다. 관찰 대상은 Instagram 전체 제품이 아니라 **프로필 헤더와 그리드의 시각 구조**다.

| 관찰 | 측정·영향 | GYEOL 적용 |
|---|---|---|
| 평평한 흰 캔버스 | 흰색 면과 1px 구분선이 카드·그림자보다 먼저 구조를 만든다 | 입력·프리뷰·공유의 기본 면은 흰색. 의미 없는 카드 외곽·그림자·그라데이션을 제거한다 |
| 짧은 상단 바 | 약 60px 높이, 하단 1px 선. 브랜드와 한두 행동만 있다 | GYEOL 이름, 뒤로 가기, 현재 단계만 둔다. Instagram 로고와 고유 아이콘은 사용하지 않는다 |
| 프로필 정보의 고정 순서 | 프로필 사진·사용자명 → bio/link → 가로 항목 → 수치 → 탭 → 사진 순서 | 연결 완료에는 프로필 사진·표시 이름·사용자명·수집 시각만 보인다. 하이라이트·팔로워 수·탭은 제품 가치가 없어 생략한다 |
| 사진이 만드는 밀도 | 3열이 화면 너비를 가득 쓰고 사진 사이 여백이 거의 없다 | 공유는 3열 full-bleed에 가깝게, 편집은 선택·이동 포커스를 위한 최소 2px 간격을 둔다 |
| 장식보다 사진 우선 | 사진 영역에 카드 제목·설명·배지가 반복되지 않는다 | 그리드에는 순서와 제외 상태만 얹고 근거·캡션 편집은 선택한 사진의 프리뷰 패널에서 처리한다 |
| 모바일 웹 타일 비율 | 관찰 화면은 약 3:4 타일이었다 | 플랫폼을 그대로 복제하지 않고 합의된 1:1 썸네일을 유지한다. 상세는 원본 비율을 사용한다 |

### 관찰 팔레트와 제품 토큰

아래 값은 해당 화면 픽셀에서 확인한 sRGB 기준이다. Instagram의 브랜드 팔레트를 복제하는 명세가 아니라, 사진 중심 화면에 필요한 명도 구조의 근거다.

| 역할 | 관찰값 | GYEOL 결정 |
|---|---:|---|
| 기본 면 | `#FFFFFF` | `surface`와 공유 페이지 배경에 사용 |
| 주 텍스트 | `#000000`, `#262626` | `ink`를 거의 검정으로 조정해 제목·캡션에 사용 |
| 보조 텍스트 | `#8E8E8E` | 수집 시각·도움말에만 사용하고 4.5:1이 필요한 본문에는 더 진한 값을 쓴다 |
| 구분선 | `#DBDBDB`, `#DBDFE4` | 1px `line`으로 사용 |
| 링크 | `#00376B` | 외부 URL에만 사용 |
| 관찰된 행동색 | `#4A5DF9` | 그대로 복제하지 않는다. GYEOL 행동은 기존 초록 `#2E5E4E`를 사용한다 |

사진의 색이 주 팔레트다. GYEOL의 종이색은 캐릭터와 빈 상태의 작은 면에만 남기고 전체 화면을 누렇게 덮지 않는다. 보라색은 비움·제외 제안처럼 의미가 있는 상태에만 쓴다.

## 화면 계약

1. **공개 프로필 연결:** 한 화면에 URL 한 개, 공개 범위 안내, 연결 행동을 둔다. 성공하면 프로필 요약과 `본인 소유 여부는 확인하지 않았어요`를 함께 표시한다. 비공개·접근 실패·미확인은 서로 다른 복구 문구를 가진다.
2. **사진 3~15장:** 모바일 3열 선택 그리드와 큰 `+` 타일을 사용한다. 파일 선택 뒤 순서 번호를 보이고, 2장 이하와 16장 이상은 현재 선택을 지우지 않은 채 이유를 설명한다.
3. **선택 프롬프트:** 사진 뒤의 한 개 textarea다. 비워도 다음 단계로 갈 수 있으며, 계정 연결과 프롬프트의 영향 범위를 짧게 표시한다.
4. **큐레이션 프리뷰:** 프로필 헤더 아래 3열 1:1 그리드를 중심에 둔다. 사진을 선택하면 원본 비율 미리보기와 캡션, 제외·복원, 앞뒤 이동을 같은 패널에서 편집한다. 드래그만 강요하지 않는다.
5. **확정·공유:** `큐레이션 확정`은 프리뷰의 현재 상태를 저장한다. 프로필 정보 포함은 기본 꺼짐이며 공유를 만드는 사용자가 직접 켠다. 공유 페이지는 선택에 따라 프로필 헤더 또는 GYEOL 일반 헤더와 그리드·사진 상세·캡션을 보여 주며 좋아요·댓글·팔로워 수와 Instagram 이동 버튼은 없다.

편집 화면은 모바일에서 한 열로 쌓고, 데스크톱에서는 최대 1152px 안에서 사진 작업영역과 입력 패널을 나란히 둔다. 공유 화면은 최대 935px까지 확장한다. 360·390·430px에서는 3열을 유지하며, 하단 확정 행동이 키보드와 safe area에 가려지지 않아야 한다. 사진 상세는 전체 화면 dialog로 열고 닫기·이전·다음에 이름과 키보드 조작을 제공한다.

## AI 생성 화면처럼 보이지 않게 하는 기준

- 대형 그라데이션, glass 효과, 둥근 카드의 반복, 과한 배지, 가짜 통계와 점수를 사용하지 않는다.
- 로딩 문구는 실제 단계만 말하며 퍼센트와 남은 시간을 꾸며내지 않는다.
- AI 결과는 완성품이 아니라 편집 가능한 큐레이션 초안으로 표시한다. 캡션 씨앗, 제외 후보와 근거를 사용자가 바꿀 수 있어야 한다.
- 캐릭터는 연결·분석·빈 상태를 돕는 한 영역에만 두고 사진 그리드와 공유 페이지의 주인공이 되지 않는다.
- 모바일 실제 사진, 긴 사용자명, 캡션 없음, 15장, 키보드 열림, 느린 요청과 오류 상태를 시각 인수에 포함한다.
