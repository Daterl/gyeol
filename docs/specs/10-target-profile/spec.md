...
| `caption_coverage` | 말수가 적·말수 적게·일부는 비워·사진만 두·몇 장만+쓰기 문맥 → `sparse` / 모든 사진에 문장·사진마다+쓰기 문맥·한 장도 비우지·전부 써·전부 채워 → `all` | 해당 enum | 0.8 |

`사진마다`와 `몇 장만`은 단독 cue가 아니다. 문장·캡션·글·한 줄·쓰기 문맥이 같은 승인 패턴 안에 있어야 한다. `하지만`이 있으면 마지막 `하지만` 뒤 절만 평가하고, 그 절의 cue가 모두 부정됐으면 필드를 생략한다. 매치 앞의 “싫은 건”과 매치 뒤의 “지는 마/지는 않/싫다”도 부정으로 본다. `한 장도 비우지`는 승인된 전체 패턴 자체일 때만 all이며, 뒤에서 그 선택을 싫다고 말하면 생략한다.
