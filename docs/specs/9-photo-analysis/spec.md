...
의존성 0개 제약 때문에 이미지 디코더를 쓸 수 없다. 대신 **JPEG 의 DC 계수만 읽는다.** DC 계수는 8×8 블록의 평균 화소값이므로(`mean = DC_dequantized / 8 + 128`), 블록 단위로 8배 축소된 실제 이미지를 얻는 것과 같다. IDCT 와 업샘플링은 필요하지 않다. AC 계수의 **값**도 쓰지 않지만, 베이스라인은 한 블록의 DC 와 63개 AC 가 같은 엔트로피 스트림에 붙어 있으므로 AC 를 **소비는 해야 한다** — 건너뛰지 않으면 두 번째 블록부터 DC 를 엉뚱한 비트 위치에서 읽는다(`review-codex.md` H1 이 잡은 결함이고, 실패가 아니라 *틀린 색*으로 나왔다). 프로그레시브의 DC first scan 에는 AC 가 없어 소비할 것이 없다. 베이스라인(SOF0/SOF1)과 프로그레시브(SOF2, DC first scan)를 모두 읽는다 — 실제 인스타 이미지가 프로그레시브 JPEG 이다(`pivot/apify-check/fixtures/images/` 확인).
...
| `composition` | **측정하지 않는다.** `full_frame` 고정값 — 아래 참조 |
| `quality_flags` | `bright_mean < 0.25` → `dark`. 그 외 플래그는 내지 않는다 |
| (참고 측정값) | 인접 블록 평균 색차 = 디테일량. 계약 필드가 아니며 `run_pipeline` 요약표에만 남긴다 |

`composition` 은 원래 최상위 버킷 점유율 ≥ 0.28 을 `negative_space` 로 매핑했다. **그 매핑을 버렸다.** 색 히스토그램에는 위치도, 연결된 빈 영역도, 피사체도 없다. 화면을 꽉 채운 흑백 체크무늬가 `negative_space` 로 나오고, 사람이 프레임을 채운 ph_04 도 `negative_space` 로 나왔다(`review-codex.md` H4). 점유율 분포 0.079~0.334 는 임계값의 출처일 뿐 구도 분류의 정답 근거가 아니다. 점유율 자체는 여전히 낸다 — **색 점유율이라고 말하는 형태로**, `describable_facts` 의 `"주요 색 #xxxxxx (점유 N%)"` 문장으로. `scale` 과 같은 처리다.
...
### 스키마가 표현하지 못하는 세 자리 — 어떻게 정직하게 처리하는가

`has_face`(필수 boolean)·`scale`(필수 enum)·`composition`(필수 enum)에는 "관측하지 못했다"를 적을 값이 없다.
...
- `composition` 은 **측정할 수 없어서 `full_frame` 고정값**으로 둔다. 색 점유율로 `negative_space` 를 단정하던 것을 버렸다(위 4절 참조). `scale` 과 같은 이유이고 같은 경계를 적용한다: **#12 를 포함한 downstream 은 휴리스틱 경로의 `composition` 을 공간적 근거로 읽어서는 안 된다.** `describable_facts` 에 구도 문장은 넣지 않는다. 모델 경로의 `composition` 은 모델이 실제로 본 것이므로 그대로 쓴다.

세 자리 모두 스키마에 `unknown` 이 없는 탓이다. **스키마를 바꾸지 않고** 변경 요청을 PR 본문에 남긴다.
...
SVG 는 골든 fixture 가 쓰는 **단 하나의 형태**만 측정한다:

```
<svg width=W height=H><rect width=W height=H fill=#rrggbb/>[<text …>…</text>]</svg>
```

캔버스를 **전부 덮는 rect 하나**와 (있으면) **보이는 `<text>` 하나**. 그 밖은 전부 `null` → `ANALYSIS_UNAVAILABLE`.

`fill` 속성 **개수**를 세고 표시 여부와 무관하게 `<text>` 를 읽던 앞 버전은, 흰 카드에 면적 0 인 검정 rect 를 넣으면 "검정 점유 50%" 를 내고 `display="none"` 인 글자를 사실로 냈으며, 닫히지도 않은 SVG 도 측정했다(`review-codex.md` H3). **`fill` 속성은 면적이 아니고, 보이지 않는 요소는 관측이 아니다.** 이 레포에는 래스터라이저가 없으므로 면적을 계산할 방법이 rect 기하 하나뿐이고, 그래서 지원 형태를 그 하나로 좁혔다. 일반 SVG 렌더링은 하지 않으며 이 경로를 그 방향으로 키워서도 안 된다. `<text>` 는 XML 엔티티를 풀어서(`&amp;` → `&`) 낸다.
...
5. `composition` 에도 `unknown` 이 없다 → 픽셀만 본 경로가 구도를 단정할 수밖에 없다. `scale` 과 같은 요청이다.

다섯 건 모두 **`CLAUDE.md` 4-2절 절차(L 크기, 양쪽 사람 승인)** 대상이므로 이 PR 에서 손대지 않는다.
