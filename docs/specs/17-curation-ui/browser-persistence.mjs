...
try{
 const page=await browser.newPage({viewport:{width:390,height:844},hasTouch:true,reducedMotion:'reduce'});const feeds=[];const generations=[];const errors=[];page.on('pageerror',e=>errors.push(e.message));page.on('dialog',d=>d.accept());
 ...
 return json({...feed,curation:{schema_version:'1.0',profile_snapshot_id:b.profile_snapshot_id,profile:{display:{username:b.profile_url.includes('profile_a')?'profile_a':'profile_b'},snapshot_id:'offline-snapshot',source_url:b.profile_url,collected_at:'2026-09-18T10:00:00.000Z',expires_at:'2026-09-19T10:00:00.000Z',ownership_verified:false,evidence_refs:{}},prompt:{text:b.prompt||null,evidence:[]},slots:feed.feed.slots.map(s=>({photo_id:s.photo_id,position:s.position,included:true,exclusion_candidate:{recommended:false,reason:null,evidence:[]}}))}});
 }
 ...
 const file=readFileSync(new URL('../../../public/images/gyeol-character/default.webp', import.meta.url));await page.locator('input[type=file]').first().setInputFiles([1,2,3,4].map(i=>({name:`photo${i}.webp`,mimeType:'image/webp',buffer:file})));
 await page.getByText('4장 선택',{exact:true}).waitFor();await page.getByLabel('원하는 느낌 (선택)').fill('saved user prompt');await page.getByLabel(/사진 분석·문장 생성에 유료/).check();await page.getByRole('button',{name:'큐레이션 만들기',exact:true}).click();await page.getByLabel('기록의 제목').waitFor();
 ...
 let saved=await page.evaluate(()=>JSON.parse(localStorage.getItem('gyeol.editor.draft.v1')));
 ...
 assert.equal(saved.curationState.confirmed.output.slots.length,3);
 assert.equal(saved.curationState.confirmed.profileSnapshotId,'reference-a');
 assert.equal('profile' in saved.curationState.confirmed,false);
 assert.equal(JSON.stringify(saved.curationState.confirmed).includes('profile_a'),false);
 await page.getByRole('button',{name:'큐레이션 확정',exact:true}).click();
 await page.waitForFunction(()=>JSON.parse(localStorage.getItem('gyeol.editor.draft.v1')).curationState.confirmed.output.title==='later unconfirmed title');
 saved=await page.evaluate(()=>JSON.parse(localStorage.getItem('gyeol.editor.draft.v1')));
 ...
 const binaries=await imageState(); assert.equal(binaries.length,1); assert.equal(binaries[0].images.length,4); assert.ok(binaries[0].images.every(i=>i.type==='image/webp'&&i.size>0));
 ...
 const beforeLegacy=await page.evaluate(()=>JSON.parse(localStorage.getItem('gyeol.editor.draft.v1')));
 const legacy=structuredClone(beforeLegacy); delete legacy.curationState.confirmed.profileSnapshotId;
 legacy.curationState.confirmed.output.slots=legacy.curationState.confirmed.output.slots.slice(0,2);
 const legacyIds=new Set(legacy.curationState.confirmed.output.slots.map(slot=>slot.photo_id));
 legacy.curationState.confirmed.crops=Object.fromEntries(Object.entries(legacy.curationState.confirmed.crops).filter(([id])=>legacyIds.has(id)));
 legacy.curationState.confirmed.profile={source_url:'https://www.instagram.com/profile_a/',username:'profile_a',collected_at:'2026-09-18T10:00:00.000Z'};
 await page.evaluate(value=>localStorage.setItem('gyeol.editor.draft.v1',JSON.stringify(value)),legacy);
 const legacyBinaries=await imageState(); await page.reload({waitUntil:'networkidle'}); await page.getByLabel('기록의 제목').waitFor();
 assert.deepEqual(await page.evaluate(()=>JSON.parse(localStorage.getItem('gyeol.editor.draft.v1'))),legacy);
 assert.equal(await page.getByLabel('기록의 제목').inputValue(),'later unconfirmed title');
 assert.equal(await page.getByLabel('공유에 공개 프로필 정보 포함 (기본 꺼짐)').isChecked(),true);
 assert.equal(await page.getByText('확정본:',{exact:false}).count(),0); assert.deepEqual(await imageState(),legacyBinaries);
 await page.getByLabel('기록의 제목').fill('migrated autosave title');
 await page.waitForFunction(()=>{const value=JSON.parse(localStorage.getItem('gyeol.editor.draft.v1'));return value?.draft?.title==='migrated autosave title'&&value?.curationState?.confirmed===null&&!('confirmedProfileSource' in value.curationState);});
 const settled=await page.evaluate(()=>JSON.parse(localStorage.getItem('gyeol.editor.draft.v1')));
 assert.equal(settled.revision,legacy.revision);
 for(const key of ['photoIds','order','original','originalOutput','prompt','profileReference'])assert.deepEqual(settled[key],legacy[key],`migrated ${key}`);
 for(const key of ['curation','excluded','crops','profileSharing'])assert.deepEqual(settled.curationState[key],legacy.curationState[key],`migrated curation ${key}`);
 const settledBinaries=await imageState(); assert.equal(settledBinaries.length,1); assert.deepEqual(settledBinaries.flatMap(value=>value.images),legacyBinaries.flatMap(value=>value.images));
 await page.screenshot({path:resolve(outputDir,'migrated-legacy.png'),fullPage:true});
 ...
 console.log(JSON.stringify({result:'PASS',touchContext:true,differentAccountBlocked:true,freshSnapshotBlocked:true,sameReferenceResumes:true,signedProfileReferenceOnly:true,immutableThenReconfirmed:true,legacyLocalStorageIndexedDbMigration:true,reloadPreserves:['photos','normalizedWebP','prompt','title','caption','order','exclude','crop','profileSharing','confirmation reference'],restoredAndExpiredConnectionsBlocked:true,transientReadPreservesData:true,resetClearsMetadataAndIndexedDB:true,feedCalls:feeds.length,generationCalls:generations.length,consoleErrors:errors},null,2));
