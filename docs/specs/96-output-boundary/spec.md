...
모델 응답과 서버 안정화 결과는 같은 공개 문구 검사기를 통과해야 한다. 제목, 캡션, 비움 이유, evidence note에 내부 필드명·순서 규칙 문구·정규화 측정값이 있으면 `MODEL_CONTRACT`로 실패한다. 일반적인 `밝기`·`채도` 표현은 해당 사진의 `describable_facts`에 실제로 있을 때만 허용한다.
