...
**Verdict: 사람 판정 대기.** 세 번 수정해 각각 12회, 총 36회의 실모델 응답을 받았다. 설명문은 소재 조각과 질문으로 바뀌었다. 품질 게이트를 **더 이상 튜닝하지 않고 측정 방법을 고쳤다** — 원문 문자 포함 검사를 사실 정합성 검사로 바꿔 같은 36회를 새 모델 호출 없이 재채점했다. 최종 회차 소재 276개 기준 **명확한 위반 2건 · 보류 10건 · 통과 264건**이다([재채점 결과](rescore.md)). 자연어 비움 회귀는 그대로 남아 있고, 사람의 1분 내 작성 가능 판정은 여전히 **pending**이다.
...
위 문자 포함 검사로는 **진짜 남은 문제가 1건인지 8건인지 구분되지 않았다.** 그래서 프롬프트를 더 고치는 대신 채점 기준을 바꿨다. 소재 문구가 원문 문자열을 포함하는가가 아니라 **그 사진의 describable_facts로 지지되는가**를 묻는다. 지지되지 않으면 여전히 실패이고, 문자로 확정할 수 없으면 통과가 아니라 보류로 남긴다.

[rescore.py](rescore.py)로 기존 `run-1.json`·`run-2.json`·`run-3.json`을 다시 채점했다. **새 모델 호출 0회**이며 `python3 rescore.py`로 재현하고 `python3 rescore.py --selftest`로 판정 규칙이 뒤집히지 않았는지 확인한다. 결과는 [rescore.md](rescore.md)와 [rescore.json](rescore.json)에 있다.
...
기존 기준은 974개 소재 중 372개를 불일치로 셌다. 새 기준으로는 명확한 위반 10건뿐이고, 그중 사진이 지지하지 않는 주장은 최종 회차의 `반려견` 2건이다. 나머지 8건은 `숄더백`→`가방`, `개`→`강아지`처럼 사실은 맞고 낱말만 바꾼 경우로, 동의어 사전을 두지 않은 이 기준이 위반으로 떨어뜨린 것이다.

사진을 직접 열고 만든 [s4-audit.json](s4-audit.json)과 최종 회차 276개 소재를 맞춰 보면 264건이 일치하고, **사진 대조가 위반·보류로 본 것을 재채점이 통과로 올린 경우는 0건**이다. 기준을 느슨하게 만들지 않았다는 근거다. 엇갈린 12건의 내역은 [rescore.md](rescore.md)에 있다.

이 기준은 사진을 보지 않고 관측 사실 텍스트만 쓴다. 관측 자체가 틀렸다면 재채점도 같이 틀린다. 동의어·상위어를 모르고, 핵심어를 마지막 어절로 단순히 잡으며, 색 수식어 귀속을 어순 사슬로만 본다. 한계는 [rescore.md](rescore.md) 마지막 절에 적었다.
