...
기존 문서의 바닐라 JS·프레임워크 미사용 결정은 이 ADR로 대체한다. HTML은 시각·흐름 참고 자료이며 샘플 분석 로직을 실제 AI로 취급하지 않는다. 한 화면 입력과 사진만으로 시작하는 제품 동작은 [ADR-0001](0001-single-screen-photo-only-entry.md)을 따른다.

## 작업 그래프

- 노드: 최상위 엄브렐라 → 기존 E1~E4 엄브렐라 → 한 AI Session/한 PR의 실행 작업.
...
기존 이슈 번호·담당·진행 증거는 유지한다. 누락된 실행 단위만 추가하며 별도 그래프 DB나 스케줄러는 만들지 않는다. 실제 생성된 노드·관계는 [AI Session 실행 문서](../ai-session-workflow.md)에 연결한다.
