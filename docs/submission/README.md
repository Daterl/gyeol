...
[factuality-audit.md](factuality-audit.md) — 실사진 15장 1벌의 실제 모델 생성 결과(타이틀 2 + 캡션 22 + 비움 이유 8, 총 32문장)를 사진의 `describable_facts` 와 한 줄씩 대조한 표다. **사진에 없는 장소·인물·시간이 들어간 문장은 0개**, 관측 사실에 근거 없는 타이틀 2개, 있는 사실을 어긋나게 옮긴 문장 3개다. 다시 만들려면 `node scripts/factuality-audit.mjs`. 판정 주체는 에이전트이고 원본 사진 이미지 대조와 사람 인수는 PENDING이다.
